...
REM ----------------------------------------------------------------------------
REM  JOB-ID      : MSTB992
REM  BATCH-ID    : MSTB992-407
REM  IF          : MDware => POS
REM  IF-NAME     : Payment type
REM  FUNCTION    : RENAME
REM  CRATOR      : G.SASAKI
REM  CREATE DATE : 2016/10/12
REM  UPDATE      : 2020/08/18
REM ----------------------------------------------------------------------------
REM  1-1. COMMON-SET
REM ----------------------------------------------------------------------------
REM (1)BATCH-ID 
SET BAT_ID=MSTB992-407
...
REM   
REM   送信先フォルダ例: H:\POS\q5\mdware\LIVE\download\000001 （店舗単位フォルダ）
REM   送信ファイル   例: L0706010.001_noncompleted
REM ----------------------------------------------------------------------------
REM (1)RENAME-TARGET-DIR
REM SET TARGET_DIR=H:\POS\q5\mdware\download\L\
SET TARGET_DIR=%1

REM (2)RENAME-TARGET-FILE-NAME
REM SET TARGETFILENAME=L*_noncompleted
SET TARGETFILENAME=L*%TARGET_DIR:~-5,1%.%TARGET_DIR:~-4,3%_noncompleted

REM (3)COPY-FROM-DIR 
SET SND_FROM_DIR=E:\mdware_law\datas\MKV\POS\q5\mdware\download\L\
...
echo [RENAME PAYMENT-TYPE FILE] >>%LOG_DIR%%LOG_FILE%
...
IF not exist %SND_FROM_DIR%\%TARGETFILENAME% (
    echo [1-2][NG][ NO RENAME-FIL] >>%LOG_DIR%%LOG_FILE%
    echo. >>%LOG_DIR%%LOG_FILE%
    goto error
)
...
echo.>>%LOG_DIR%%LOG_FILE%
CD %BATCHF_HOME%
type nul > %LOG_DIR%POS-IF-NG\L-File-RENAME-NG.Store-CD-%TARGET_DIR:~-7,6%
REM exit 1
exit /b

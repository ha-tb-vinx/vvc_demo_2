...
echo [1][START][COPY DEPT FILE]                        >>%LOG_DIR%%LOG_FILE%

   for /D %%F in (%DirPath%*) do (
      SET senddir=%%F
      echo COPY-TO-DIR --- !senddir!\                  >>%LOG_DIR%%LOG_FILE%
      
      call E:\mdware_law\batch\master\batch\batfiles\MSTB992-302.bat !senddir!\
      IF !ERRORLEVEL! NEQ 0 goto ABEND
     )

echo [1][E N D][COPY DEPT FILE]                        >>%LOG_DIR%%LOG_FILE%

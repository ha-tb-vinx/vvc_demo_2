...
REM SET TARGET_DIR=H:\POS\q5\mdware\download\K\
SET TARGET_DIR=%1

REM (2)RENAME-TARGET-FILE-NAME
REM SET TARGETFILENAME=K*_noncompleted
SET TARGETFILENAME=K*%TARGET_DIR:~-5,1%.%TARGET_DIR:~-4,3%_noncompleted

REM (3)COPY-FROM-DIR 
SET SND_FROM_DIR=E:\mdware_law\datas\MKV\POS\q5\mdware\download\K\
...
echo [RENAME DISCOUNT-TYPE-EMERGENCY FILE] >>%LOG_DIR%%LOG_FILE%
...
echo.>>%LOG_DIR%%LOG_FILE%
CD %BATCHF_HOME%
type nul > %LOG_DIR%POS-IF-NG\K-File-RENAME-NG.Store-CD-%TARGET_DIR:~-7,6%
REM exit 1
exit /b

...
REM   送信ファイル   例: C0706010.001_noncompleted
REM ----------------------------------------------------------------------------
REM (1)COPY-FROM-DIR 
SET SND_FROM_DIR=E:\mdware_law\datas\MKV\POS\q5\mdware\download\C\

REM (2)COPY-FROM-FILE-NAME
REM SET INFILENAME=C*_noncompleted

REM (3)COPY-TO-DIR 
REM SET SND_TO_DIR=H:\POS\q5\mdware\download\C\

SET SND_TO_DIR=%1
SET INFILENAME=C*%SND_TO_DIR:~-5,1%.%SND_TO_DIR:~-4,3%_noncompleted
...
echo [COPY SUB-CLASS FILE] >>%LOG_DIR%%LOG_FILE%
...
CD %BATCHF_HOME%
rem exit 1
type nul > %LOG_DIR%POS-IF-NG\C-File-NG.Store-CD-%SND_TO_DIR:~-7,6%
exit /b

...
REM SET TARGET_DIR=H:\POS\q5\mdware\download\D\
SET TARGET_DIR=%1

REM (2)RENAME-TARGET-FILE-NAME
REM SET TARGETFILENAME=D*_noncompleted
SET TARGETFILENAME=D*%TARGET_DIR:~-5,1%.%TARGET_DIR:~-4,3%_noncompleted
...
echo [RENAME DIV FILE] >>%LOG_DIR%%LOG_FILE%
...
echo.>>%LOG_DIR%%LOG_FILE%
CD %BATCHF_HOME%
type nul > %LOG_DIR%POS-IF-NG\D-File-RENAME-NG.Store-CD-%TARGET_DIR:~-7,6%
REM exit 1
exit /b

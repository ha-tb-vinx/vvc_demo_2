...
REM   送信ファイル   例: I0706010.001_noncompleted
REM ----------------------------------------------------------------------------
REM (1)COPY-FROM-DIR 
SET SND_FROM_DIR=E:\mdware_law\datas\MKV\POS\q5\mdware\download\I\

REM (2)COPY-FROM-FILE-NAME
REM SET INFILENAME=I*_noncompleted

REM (3)COPY-TO-DIR 
REM SET SND_TO_DIR=H:\POS\q5\mdware\download\I\

SET SND_TO_DIR=%1
SET INFILENAME=I*%SND_TO_DIR:~-5,1%.%SND_TO_DIR:~-4,3%_noncompleted
...
echo [COPY ITEM FILE] >>%LOG_DIR%%LOG_FILE%
...
CD %BATCHF_HOME%
rem exit 1
type nul > %LOG_DIR%POS-IF-NG\I-File-NG.Store-CD-%SND_TO_DIR:~-7,6%
exit /b

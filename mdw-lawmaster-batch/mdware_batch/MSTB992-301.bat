...
REM ----------------------------------------------------------------------------
REM  JOB-ID      : MSTB992
REM  BATCH-ID    : MSTB992-301
REM  IF          : MDware => POS
REM  IF-NAME     : Division
REM  FUNCTION    : COPY
REM  CRATOR      : T.YAJIMA
REM  CREATE DATE : 2017/04/18
REM  UPDATE      : 2020/07/25
REM ----------------------------------------------------------------------------
REM  1-1. COMMON-SET
REM ----------------------------------------------------------------------------
REM (1)BATCH-ID 
SET BAT_ID=MSTB992-301
...
REM   送信ファイル   例: D0706010.001_noncompleted
REM ----------------------------------------------------------------------------
REM (1)COPY-FROM-DIR 
SET SND_FROM_DIR=E:\mdware_law\datas\MKV\POS\q5\mdware\download\D\

REM (2)COPY-FROM-FILE-NAME
REM SET INFILENAME=D*_noncompleted

REM (3)COPY-TO-DIR 
REM SET SND_TO_DIR=H:\POS\q5\mdware\download\D\

SET SND_TO_DIR=%1
SET INFILENAME=D*%SND_TO_DIR:~-5,1%.%SND_TO_DIR:~-4,3%_noncompleted
...
echo [COPY DIV FILE] >>%LOG_DIR%%LOG_FILE%
...
IF not exist %SND_FROM_DIR%%INFILENAME% (
    echo [1-3][OK][ NO COPY-FROM-FIL] >>%LOG_DIR%%LOG_FILE%
    echo. >>%LOG_DIR%%LOG_FILE%
    goto end
)
...
CD %BATCHF_HOME%
rem exit 1
type nul > %LOG_DIR%POS-IF-NG\D-File-NG.Store-CD-%SND_TO_DIR:~-7,6%
exit /b

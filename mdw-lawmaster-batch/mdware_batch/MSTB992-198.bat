...
REM ----------------------------------------------------------------------------
REM  JOB-ID      : MSTB992
REM  BATCH-ID    : MSTB992-198
REM  IF          : MDware => POS
REM  IF-NAME     : Payment type
REM  FUNCTION    : BACKUP-E:drive
REM  CRATOR      : S.NAKAZATO
REM  CREATE DATE : 2016/12/09
REM  UPDATE      : 2017/05/18 S.Takayama #5059
REM ----------------------------------------------------------------------------
REM  1-1. COMMON-SET
REM ----------------------------------------------------------------------------
REM (1)BATCH-ID 
SET BAT_ID=MSTB992-198
...
REM ----------------------------------------------------------------------------
REM (1)COPY-FROM-DIR 
SET SND_FROM_DIR=E:\mdware_law\datas\MKV\POS\q5\mdware\download_all\L\

REM (2)COPY-FROM-FILE-NAME
SET INFILENAME=L*

REM (3)COPY-TO-DIR 
mkdir E:\mdware_law\datas\MKV\POS\q5\mdware\download_all\L\backup\%BKUPDIR%
SET SND_TO_DIR=E:\mdware_law\datas\MKV\POS\q5\mdware\download_all\L\backup\%BKUPDIR%\
...
echo [Edrive BKUP Payment type] >>%LOG_DIR%%LOG_FILE%

...
REM ----------------------------------------------------------------------------
REM  JOB-ID      : MSTB920
REM  BATCH-ID    : MSTB920-306-OYA
REM  IF          : MDware => POS
REM  IF-NAME     : Discount
REM  FUNCTION    : COPY
REM  CRATOR      : G.SASAKI
...
echo [1][START][COPY DISCOUNT FILE]                    >>%LOG_DIR%%LOG_FILE%

   for /D %%F in (%DirPath%*) do (
      SET senddir=%%F
      echo COPY-TO-DIR --- !senddir!\                  >>%LOG_DIR%%LOG_FILE%
      
      call E:\mdware_law\batch\master\batch\batfiles\MSTB920-306.bat !senddir!\
      IF !ERRORLEVEL! NEQ 0 goto ABEND
     )

echo [1][E N D][COPY DISCOUNT FILE]                    >>%LOG_DIR%%LOG_FILE%

...
REM SET INFILENAME=L*_noncompleted

REM (3)COPY-TO-DIR 
REM SET SND_TO_DIR=H:\POS\q5\mdware\download\L\

SET SND_TO_DIR=%1
SET INFILENAME=L*%SND_TO_DIR:~-5,1%.%SND_TO_DIR:~-4,3%_noncompleted
...
echo [COPY PAYMENT FILE] >>%LOG_DIR%%LOG_FILE%
...
CD %BATCHF_HOME%
rem exit 1
type nul > %LOG_DIR%POS-IF-NG\L-File-NG.Store-CD-%SND_TO_DIR:~-7,6%
exit /b

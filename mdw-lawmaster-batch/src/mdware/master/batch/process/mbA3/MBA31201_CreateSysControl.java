...
/**
 * <p>タイトル:情報分析用システムコントロールマスタ生成処理</p>
 * <p>著作権: Copyright (c) </p>
...
public class MBA31201_CreateSysControl {

	// CSVファイル名（システムコントロールマスタ）
	private static final String FILE_NAME		= "IF_SYSTEM_CONTROL.csv";
	// 区切り文字（カンマ区切り）
	private static final String SPLIT_CODE	= ",";
	// 改行文字
	private static final String RETURN_CODE	= System.getProperty("line.separator");

	private static final String TAISYOGAI_SUB	= "'TANAOROSHI','DAILYORDER','KAIKAKE','SHIIRE','SEISENEDI','MASTER','URIAGE','JISEKI','HANSOKU','AUTOORDER','MST','YOSAN','FICOMMON'";
	 ...
	 * @param dataBase
	 */
	public MBA31201_CreateSysControl( MasterDataBase db ) {
	 ...
	 * コンストラクタ
	 */
	public MBA31201_CreateSysControl() {
	...
	private String getSelDataSQL() {
		...
		sb.append( "SELECT " );
		sb.append( "    TRIM(SUBSYSTEM_ID)   AS SUBSYSTEM_ID, " );
		sb.append( "    TRIM(PARAMETER_ID)   AS PARAMETER_ID, " );
		// 2015/10/16 Sou OHPA用システムコントロールデータ修正 Start
		//sb.append( "    TRIM(PARAMETER_TX)   AS PARAMETER_TX, " );
		sb.append( " '\"' || TRIM(PARAMETER_TX) || '\"' AS PARAMETER_TX, " );
		// 2015/10/16 Sou OHPA用システムコントロールデータ修正 End
		sb.append( "    TRIM(NO_CHACHE_FG)   AS NO_CHACHE_FG, " );
		sb.append( "    TRIM(SYUSEI_KANO_FG) AS SYUSEI_KANO_FG, " );
		sb.append( "    TRIM(PLACE_NB)       AS PLACE_NB, " );
		sb.append( "    TRIM(DATA_KB)        AS DATA_KB, " );
		// 2015/11/05 togashi OHPA用システムコントロールデータ修正 Start
		//sb.append( "    TRIM(EXPLANATION)    AS EXPLANATION " );
		sb.append( " '\"' || TRIM(EXPLANATION) || '\"' AS EXPLANATION " );
		// 2015/11/05 togashi OHPA用システムコントロールデータ修正 End
		sb.append( "FROM " );
		sb.append( "    TMP_SYSTEM_CONTROL " );
		sb.append( "WHERE " );
		sb.append( "    SUBSYSTEM_ID NOT IN (" + TAISYOGAI_SUB + " )");
		sb.append( "ORDER BY " );
		sb.append( "    SUBSYSTEM_ID, " );
	...
	private StringBuffer createRowData(ResultSet rs) throws SQLException, Exception{
		...
		// サブシステムID
		if( rs.getString("SUBSYSTEM_ID") != null ){
			sb.append( rs.getString("SUBSYSTEM_ID").trim() );
		}
		sb.append( SPLIT_CODE );

		// パラメータID
		if( rs.getString("PARAMETER_ID") != null  ){
			sb.append( rs.getString("PARAMETER_ID").trim() );
		}
		sb.append( SPLIT_CODE );

		// パラメータ値
		if( rs.getString("PARAMETER_TX") != null ){
			sb.append( rs.getString("PARAMETER_TX").trim() );
		}
		sb.append( SPLIT_CODE );

		// キャッシュフラグ
		if( rs.getString("NO_CHACHE_FG") != null ){
			sb.append( rs.getString("NO_CHACHE_FG").trim() );
		}
		sb.append( SPLIT_CODE );

		// 修正可能フラグ
		if( rs.getString("SYUSEI_KANO_FG") != null ){
			sb.append( rs.getString("SYUSEI_KANO_FG").trim() );
		}
		sb.append( SPLIT_CODE );

		// 桁数
		if( rs.getString("PLACE_NB") != null ){
			sb.append( rs.getString("PLACE_NB").trim() );
		}
		sb.append( SPLIT_CODE );

		// データ型
		if( rs.getString("DATA_KB") != null ){
			sb.append( rs.getString("DATA_KB").trim() );
		}
		sb.append( SPLIT_CODE );

		// 説明
		if( rs.getString("EXPLANATION") != null ){
			sb.append( rs.getString("EXPLANATION").trim() );
		}
		sb.append( SPLIT_CODE );

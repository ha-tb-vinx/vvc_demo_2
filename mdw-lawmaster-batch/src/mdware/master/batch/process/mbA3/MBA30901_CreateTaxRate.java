...
public class MBA30901_CreateTaxRate {

	// CSVファイル名（税率マスタ）
	private static final String FILE_NAME		= "IF_R_TAX_RATE.csv";
	// 区切り文字（カンマ区切り）
	 ...
	 * @param dataBase
	 */
	public MBA30901_CreateTaxRate( MasterDataBase db ) {
	...
	private String getSelDataSQL() {
		...
		sb.append("    TRIM(TAX_RT)      AS TAX_RT ");
		sb.append("FROM ");
		sb.append("    TMP_R_TAX_RATE ");
		sb.append("ORDER BY " );
	...
	private StringBuffer createRowData(ResultSet rs) throws SQLException, Exception{
		...
		// 税率
		if( rs.getString("TAX_RT") != null ){
			sb.append( rs.getString("TAX_RT").trim() );
		}
		sb.append( SPLIT_CODE );

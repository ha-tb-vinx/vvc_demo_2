...
public class MBA30201_CreateBunrui2 {

	// CSVファイル名（分類２マスタ）
	private static final String FILE_NAME		= "IF_R_BUNRUI2.csv";
	// 区切り文字（カンマ区切り）
	 ...
	 * @param dataBase
	 */
	public MBA30201_CreateBunrui2( MasterDataBase db ) {
	...
	private String getSelDataSQL() {
		...
		ssb.append( "    TRIM(BUNRUI2_CD)      AS BUNRUI2_CD, " );
		ssb.append( "    TRIM(MAX(BUNRUI1_CD)) AS BUNRUI1_CD " );
		ssb.append( "FROM " );
		ssb.append( "    TMP_R_SYOHIN_TAIKEI " );
		ssb.append( "GROUP BY " );
		ssb.append( "    SYSTEM_KB, " );
		ssb.append( "    BUNRUI2_CD " );
		...
		//sb.append( "    RN.KANJI_NA    AS BUNRUI2_KANJI_NA, " );
		//sb.append( "    RN.KANA_NA     AS BUNRUI2_KANA_NA, " );
		sb.append( "  '\"'  ||  RN.KANJI_NA  || '\"'  AS BUNRUI2_KANJI_NA, " );
		sb.append( "  '\"'  ||  RN.KANA_NA   || '\"'  AS BUNRUI2_KANA_NA, " );
		...
		sb.append( "ON " );
		sb.append( "    RN.SYUBETU_NO_CD = '" ).append( MessageUtil.getMessage(mst000101_ConstDictionary.BUNRUI2, userLocal) ).append( "' " );
		sb.append( "AND " );
		sb.append( "    RN.CODE_CD = RST.BUNRUI2_CD " );
		sb.append( "AND " );
	...
	private StringBuffer createRowData(ResultSet rs) throws SQLException, Exception{
		...
		// 分類２名称（漢字）
		if( rs.getString("BUNRUI2_KANJI_NA") != null ){
			sb.append( rs.getString("BUNRUI2_KANJI_NA").trim() );
		}
		sb.append( SPLIT_CODE );

		// 分類２名称（カナ）
		if( rs.getString("BUNRUI2_KANA_NA") != null ){
			sb.append( rs.getString("BUNRUI2_KANA_NA").trim() );
		}
		sb.append( SPLIT_CODE );

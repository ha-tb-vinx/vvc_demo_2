...
/**
 * <p>タイトル:情報分析用システムディクショナリマスタ生成処理</p>
 * <p>著作権: Copyright (c) </p>
 * <p>会社名: Vinculum Japan Corp.</p>
 * @author A.Okada
 * @version 1.00 2015/10/16 Sou エンコーディング指定
 */

public class MBA31301_CreateSysDictionary {

	// CSVファイル名（システムディクショナリマスタ）
	private static final String FILE_NAME		= "IF_R_DICTIONARY_CONTROL.csv";
	// 区切り文字（カンマ区切り）
	 ...
	 * @param dataBase
	 */
	public MBA31301_CreateSysDictionary( MasterDataBase db ) {
	 ...
	 * コンストラクタ
	 */
	public MBA31301_CreateSysDictionary() {
	...
	private String getSelDataSQL() {
		...
		sb.append( "    TRIM(DICTIONARY_ID) AS DICTIONARY_ID, " );
		// 2015/10/21 kou OHPA用システムコントロールデータ修正 Start
		//sb.append( "    TRIM(PARAMETER_TX)   AS PARAMETER_TX " );
		sb.append( " '\"' || TRIM(PARAMETER_TX) || '\"' AS PARAMETER_TX " );
		// 2015/10/21 kou OHPA用システムコントロールデータ修正 End
		sb.append( "FROM " );
		sb.append( "    TMP_R_DICTIONARY_CONTROL " );
		sb.append( "ORDER BY " );
	...
	private StringBuffer createRowData(ResultSet rs) throws SQLException, Exception{
		...
		// ディクショナリＩＤ）
		if( rs.getString("DICTIONARY_ID") != null ){
			sb.append( rs.getString("DICTIONARY_ID").trim() );
		}
		sb.append( SPLIT_CODE );

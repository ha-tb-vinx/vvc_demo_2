...
public class MBA31701_CreateMessage {

	// CSVファイル名（店別商品データ）
	private static final String FILE_NAME		= "IF_R_MESSAGE.csv";
	// 区切り文字（カンマ区切り）
	 ...
	 * @param dataBase
	 */
	public MBA31701_CreateMessage( MasterDataBase db ) {
	...
	private String getSelDataSQL() {
		...
		sb.append("    TRIM(MESSAGE_ID)    AS MESSAGE_ID, ");
		//カンマデータ対応2015.11.04.Kanno
		//sb.append("    TRIM(MESSAGE_TX)    AS MESSAGE_TX ");
		sb.append(" '\"' || TRIM(MESSAGE_TX) || '\"' AS MESSAGE_TX ");
		sb.append("FROM ");
		sb.append("    TMP_R_MESSAGE ");
		sb.append("ORDER BY " );
	...
	private StringBuffer createRowData(ResultSet rs) throws SQLException, Exception{
		...
		if( rs.getString("MESSAGE_ID") != null  ){
			sb.append( rs.getString("MESSAGE_ID").trim() );
		}
		sb.append( SPLIT_CODE );

		// メッセージ内容
		if( rs.getString("MESSAGE_TX") != null  ){
			sb.append( rs.getString("MESSAGE_TX").trim() );
		}
		sb.append( SPLIT_CODE );

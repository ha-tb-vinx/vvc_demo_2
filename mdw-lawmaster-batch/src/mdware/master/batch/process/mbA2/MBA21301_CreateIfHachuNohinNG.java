...
/**
 * <p>タイトル:MSTB111130_IF店舗納品不可日マスタ作成処理</p>
 * <p>著作権: Copyright (c) </p>
...
public class MBA21301_CreateIfHachuNohinNG{
	 ...
	 * @param dataBase
	 */
	public MBA21301_CreateIfHachuNohinNG( MasterDataBase db ) {
	 ...
	 * コンストラクタ
	 */
	public MBA21301_CreateIfHachuNohinNG() {
	...
	public void execute() throws Exception {
		...
		try {
			...
			try{
				...
				writeLog(Level.ERROR_INT, "内部IFスキーマ名が設定されていないため、TMP店舗納品不可日マスタはIFされません。");
				return;
			}

			// テーブルクリア
			writeLog(Level.INFO_INT, "IFテーブルのデータクリアを開始します。");
			DBUtil.truncateTable(db, ifSchema, "IF_R_HACHUNOHIN_NG");
	...
	public String getInsertSql(String timeStamp) throws SQLException, Exception {
		...
		// SQL作成
		sb.append("INSERT /*+ APPEND */ INTO IF_R_HACHUNOHIN_NG NOLOGGING (");
		sb.append("	TENPO_CD, ");
		sb.append("	YMD_DT, ");
		sb.append("	NOHIN_NG_FG, ");
		sb.append("	TEN_CLOSED_FG, ");
		...
		sb.append("	TRIM(TEN_CLOSED_FG), ");
		sb.append("	'").append(timeStamp).append("' ");
		sb.append("FROM TMP_R_HACHUNOHIN_NG ");

...
/**
 * <p>タイトル:MSTB111030_IF分類５マスタ作成処理</p>
 * <p>著作権: Copyright (c) </p>
...
public class MBA20301_CreateIfBunrui5 {
	 ...
	 * @param dataBase
	 */
	public MBA20301_CreateIfBunrui5( MasterDataBase db ) {
	 ...
	 * コンストラクタ
	 */
	public MBA20301_CreateIfBunrui5() {
	...
	public void execute() throws Exception {
		...
		try {
			...
			try{
				...
				writeLog(Level.ERROR_INT, "内部IFスキーマ名が設定されていないため、TMP分類５マスタはIFされません。");
				return;
			}

			// テーブルクリア
			writeLog(Level.INFO_INT, "IFテーブルのデータクリアを開始します。");
			DBUtil.truncateTable(db, ifSchema, "IF_R_BUNRUI5");
	...
	public String getInsertSql(String timeStamp) throws SQLException, Exception {
		...
		sb.append("INSERT /*+ APPEND */ INTO IF_R_BUNRUI5 NOLOGGING ( ");
		sb.append("	SYSTEM_KB, ");
		sb.append("	BUNRUI5_CD, ");
		sb.append("	BUNRUI5_KANJI_NA, ");
		sb.append("	BUNRUI5_KANA_NA, ");
		...
		sb.append("	TRIM(RS.BUNRUI5_CD), ");
		sb.append("	TRIM(NC.KANJI_NA), ");
		sb.append("	TRIM(NC.KANA_NA), ");
		sb.append("	MAX(TRIM(RS.BUNRUI2_CD)), ");
		...
		sb.append("	ON NC.SYUBETU_NO_CD = '").append(MessageUtil.getMessage(mst000101_ConstDictionary.BUNRUI5, userLocal)).append("' ");
		sb.append("	AND TRIM(NC.CODE_CD) = TRIM(RS.SYSTEM_KB)  || TRIM(RS.BUNRUI5_CD) ");

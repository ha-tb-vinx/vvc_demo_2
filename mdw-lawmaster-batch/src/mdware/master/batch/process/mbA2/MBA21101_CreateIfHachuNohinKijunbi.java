...
/**
 * <p>タイトル:MSTB111110_IF発注納品基準日マスタ作成</p>
 * <p>著作権: Copyright (c) </p>
...
public class MBA21101_CreateIfHachuNohinKijunbi{
	 ...
	 * @param dataBase
	 */
	public MBA21101_CreateIfHachuNohinKijunbi( MasterDataBase db ) {
	 ...
	 * コンストラクタ
	 */
	public MBA21101_CreateIfHachuNohinKijunbi() {
	...
	public void execute() throws Exception {
		...
		try {
			...
			try{
				...
				writeLog(Level.ERROR_INT, "内部IFスキーマ名が設定されていないため、TMP発注納品基準日マスタはIFされません。");
				return;
			}

			// テーブルクリア
			writeLog(Level.INFO_INT, "IFテーブルのデータクリアを開始します。");
			DBUtil.truncateTable(db, ifSchema, "IF_R_HACHUNOHINKIJUNBI");
	...
	public String getInsertSql(String timeStamp,  String batchDate) throws SQLException, Exception {
		...
		// SQL作成
		sb.append("INSERT /*+ APPEND */ INTO IF_R_HACHUNOHINKIJUNBI NOLOGGING (");
		sb.append("YUKO_DT, ");
		sb.append("KANRI_KB, ");
		sb.append("KANRI_CD, ");
		...
		sb.append("FROM TMP_R_HACHUNOHINKIJUNBI TB1 ");
		...
		sb.append("	FROM ");
		sb.append("		TMP_R_HACHUNOHINKIJUNBI TBSUB ");
		sb.append("	WHERE ");
		sb.append("		TBSUB.YUKO_DT <= '").append( DateChanger.addDate(batchDate, SYORI_SPAN) ).append("' ");
		sb.append("		AND TBSUB.KANRI_KB = TB1.KANRI_KB ");
		sb.append("		AND TBSUB.KANRI_CD = TB1.KANRI_CD ");
		sb.append("		AND TBSUB.SIIRESAKI_CD = TB1.SIIRESAKI_CD ");

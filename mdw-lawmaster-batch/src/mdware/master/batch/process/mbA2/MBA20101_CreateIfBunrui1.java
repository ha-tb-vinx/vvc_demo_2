...
public class MBA20101_CreateIfBunrui1{
	 ...
	 * @param dataBase
	 */
	public MBA20101_CreateIfBunrui1( MasterDataBase db ) {
	 ...
	 * コンストラクタ
	 */
	public MBA20101_CreateIfBunrui1() {
	...
	public void execute() throws Exception {
		...
		try {
			...
			try{
				...
				writeLog(Level.ERROR_INT, "内部IFスキーマ名が設定されていないため、TMP分類１マスタはIFされません。");
				return;
			}

			// テーブルクリア
			writeLog(Level.INFO_INT, "IFテーブルのデータクリアを開始します。");
			DBUtil.truncateTable(db, ifSchema, "IF_R_BUNRUI1");
	...
	protected String getInsertSql(String timeStamp) {
		...
		sb.append("INSERT /*+ APPEND */ INTO IF_R_BUNRUI1 NOLOGGING ( ");
		sb.append("	SYSTEM_KB, ");
		sb.append("	BUNRUI1_CD, ");
		sb.append("	BUNRUI1_KANJI_NA, ");
		sb.append("	BUNRUI1_KANA_NA, ");
		...
		sb.append("	TRIM(RS.BUNRUI1_CD), ");
		sb.append("	TRIM(NC.KANJI_NA), ");
		sb.append("	TRIM(NC.KANA_NA), ");
		sb.append("	MAX(TRIM(RS.DAIBUNRUI2_CD)), ");
		...
		sb.append("	ON NC.SYUBETU_NO_CD = '").append(MessageUtil.getMessage(mst000101_ConstDictionary.BUNRUI1, userLocal)).append("' ");
		sb.append("	AND TRIM(NC.CODE_CD) = TRIM(RS.BUNRUI1_CD) ");

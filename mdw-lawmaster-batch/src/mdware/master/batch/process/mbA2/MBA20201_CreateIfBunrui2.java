...
/**
 * <p>タイトル:MSTB111020_IF分類２マスタ作成処理</p>
 * <p>著作権: Copyright (c) </p>
...
public class MBA20201_CreateIfBunrui2 {
	 ...
	 * @param dataBase
	 */
	public MBA20201_CreateIfBunrui2( MasterDataBase db ) {
	 ...
	 * コンストラクタ
	 */
	public MBA20201_CreateIfBunrui2() {
	...
	public void execute() throws Exception {
		...
		try {
			...
			try{
				...
				writeLog(Level.ERROR_INT, "内部IFスキーマ名が設定されていないため、TMP分類２マスタはIFされません。");
				return;
			}

			// テーブルクリア
			writeLog(Level.INFO_INT, "IFテーブルのデータクリアを開始します。");
			DBUtil.truncateTable(db, ifSchema, "IF_R_BUNRUI2");
	...
	public String getInsertSql(String timeStamp) throws SQLException, Exception {
		...
		sb.append("INSERT /*+ APPEND */ INTO IF_R_BUNRUI2 NOLOGGING ( ");
		sb.append("	SYSTEM_KB, ");
		sb.append("	BUNRUI2_CD, ");
		sb.append("	BUNRUI2_KANJI_NA, ");
		sb.append("	BUNRUI2_KANA_NA, ");
		...
		sb.append("	TRIM(RS.BUNRUI2_CD), ");
		sb.append("	TRIM(NC.KANJI_NA), ");
		sb.append("	TRIM(NC.KANA_NA), ");
		sb.append("	MAX(TRIM(RS.BUNRUI1_CD)), ");
		...
		sb.append("	ON NC.SYUBETU_NO_CD = '").append(MessageUtil.getMessage(mst000101_ConstDictionary.BUNRUI2, userLocal)).append("' ");
		sb.append("	AND TRIM(NC.CODE_CD) = TRIM(RS.BUNRUI2_CD) ");

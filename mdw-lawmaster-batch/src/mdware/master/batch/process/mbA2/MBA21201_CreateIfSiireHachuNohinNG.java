...
/**
 * <p>タイトル:MSTB111120_IF取引先発注納品不可日マスタ作成</p>
 * <p>著作権: Copyright (c) </p>
...
public class MBA21201_CreateIfSiireHachuNohinNG{
	 ...
	 * @param dataBase
	 */
	public MBA21201_CreateIfSiireHachuNohinNG( MasterDataBase db ) {
	 ...
	 * コンストラクタ
	 */
	public MBA21201_CreateIfSiireHachuNohinNG() {
	...
	public void execute() throws Exception {
		...
		try {
			...
			try{
				...
				writeLog(Level.ERROR_INT, "内部IFスキーマ名が設定されていないため、TMP取引先発注納品不可日マスタはIFされません。");
				return;
			}

			// テーブルクリア
			writeLog(Level.INFO_INT, "IFテーブルのデータクリアを開始します。");
			DBUtil.truncateTable(db, ifSchema, "IF_R_SIIRE_HACHUNOHIN_NG");
	...
	public String getInsertSql(String timeStamp) throws SQLException, Exception {
		...
		// SQL作成
		sb.append("INSERT /*+ APPEND */ INTO IF_R_SIIRE_HACHUNOHIN_NG NOLOGGING (");
		sb.append("	KANRI_KB, ");
		sb.append("	KANRI_CD, ");
		...
		sb.append("	TRIM(HACHU_NG_FG), ");
		sb.append("	TRIM(NOHIN_NG_FG), ");
		sb.append("	'").append(timeStamp).append("' ");
		sb.append("FROM TMP_R_SIIRE_HACHUNOHIN_NG ");

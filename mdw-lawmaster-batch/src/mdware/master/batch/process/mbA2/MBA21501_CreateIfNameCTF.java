...
public class MBA21501_CreateIfNameCTF{
	 ...
	 * @param dataBase
	 */
	public MBA21501_CreateIfNameCTF( MasterDataBase db ) {
	 ...
	 * コンストラクタ
	 */
	public MBA21501_CreateIfNameCTF() {
	...
	public void execute() throws Exception {
		...
		try {
			...
			try{
				...
				writeLog(Level.ERROR_INT, "内部IFスキーマ名が設定されていないため、TMP名称マスタはIFされません。");
				return;
			}

			// テーブルクリア
			writeLog(Level.INFO_INT, "IFテーブルのデータクリアを開始します。");
			DBUtil.truncateTable(db, ifSchema, "IF_R_NAMECTF");
	...
	public String getInsertSql(String timeStamp) throws SQLException, Exception {
		...
		// SQL作成
		sb.append("INSERT /*+ APPEND */ INTO IF_R_NAMECTF NOLOGGING (");
		...
		sb.append("SELECT ");
		sb.append("	TRIM(SYUBETU_NO_CD), ");
		sb.append("	TRIM(CODE_CD), ");
		sb.append("	TRIM(KANJI_NA), ");
		sb.append("	TRIM(KANA_NA), ");
		sb.append("	TRIM(KANJI_RN), ");
		sb.append("	TRIM(KANA_RN), ");
		sb.append("	TRIM(ZOKUSEI_CD), ");
		sb.append("	'").append(timeStamp).append("' ");
		sb.append("FROM TMP_R_NAMECTF ");

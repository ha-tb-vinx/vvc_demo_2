...
/**
 *  <p>タイトル: MSTB991060_IfTrePaymentMasterCsvCreate クラス</p>
 *  <p>説明: TRE向けIFファイル（支払方法マスタ）を作成する。</p>
 *  <p>著作権: Copyright (c) 2022</p>
 *  <p>会社名: VVC</p>
 *  @author VINX
 *  @Version 1.00 (2022.01.26) KHAI.NN #6497 MKH対応
 */
public class MSTB991060_IfTrePaymentMasterCsvCreate {
	/** DBインスタンス */
    ...
    // テーブル
    private static final String TABLE_WK = "WK_IF_TRE_PAYMENT_MASTER"; // WK_支払方法マスタ
     ...
     * @param dataBase
     */
    public MSTB991060_IfTrePaymentMasterCsvCreate(DataBase db) {
        this.db = db;
        if (db == null) {
     ...
     * コンストラクタ
     */
    public MSTB991060_IfTrePaymentMasterCsvCreate() {
    ...
    public void execute() throws Exception {
        ...
        try {
            ...
            String csvFileName = ResorceUtil.getInstance().getPropertie("IF_TRE_PAYMENT_MASTER_FILENAME");
            // ファイル名編集
            ...
            // IF支払ワーク抽出
            writeLog(Level.INFO_INT, "TRE向けIFファイル（支払方法マスタ）作成処理を開始します。");
            selectDataToCSV(csvFileName);
            writeLog(Level.INFO_INT, "TRE向けIFファイル（支払方法マスタ）作成処理を終了します。");
    ...
    */
    public String getStoreMasterSelectSql() {
        ...
        sb.append(" SELECT ");
        sb.append("    WITPM.SHIHARAI_SYUBETSU_SEQ ");
        sb.append("    ,WITPM.SHIHARAI_SYUBETSU_NA ");
        sb.append("    ,WITPM.UPDATE_TS ");
        sb.append(" FROM ");
        sb.append(TABLE_WK + " WITPM ");
        sb.append(" ORDER BY");
        sb.append("       WITPM.SHIHARAI_SYUBETSU_SEQ ");
    ...
    public String getStoreMasterInsertSql() {
        ...
        sb.append("    ,SHIHARAI_SYUBETSU_NA ");
        sb.append("    ,UPDATE_TS ");
        sb.append("    ) ");
        sb.append("SELECT ");
        sb.append("     DISTINCT RP.SHIHARAI_SYUBETSU_SEQ ");
        sb.append("    ,TRIM(SUBSTR(RP.SHIHARAI_SYUBETSU_SUB_NA, 1, 50)) AS SHIHARAI_SYUBETSU_NA ");
        sb.append("    ,TO_CHAR( TO_DATE(RP.UPDATE_TS, 'yyyymmddhh24miss'), 'yyyy/MM/dd hh24:mi') AS UPDATE_TS ");
        sb.append("FROM ");
        sb.append("     R_PAYMENT RP ");
		sb.append(" ORDER BY RP.SHIHARAI_SYUBETSU_SEQ ");
        return sb.toString();
    }
    ...
    private void outputCSV(ResultSet rs, String csvFileName) throws Exception {
        ...
        try {
            ...
            do {
                ...
                dtlLine.addItem(addQuarter(StringUtility.cnvNullToBlank(rs.getString("SHIHARAI_SYUBETSU_SEQ"))));
                dtlLine.addItem(addQuarter(StringUtility.cnvNullToBlank(rs.getString("SHIHARAI_SYUBETSU_NA"))));
                dtlLine.addItem(addQuarter("0"));
                dtlLine.addItem(addQuarter("0"));
            ...
            if (count > 0) {
                // 処理件数ログ出力
                writeLog(Level.INFO_INT, count + "件の支払方法マスタを作成しました。");
            } else {
                ...
                // 処理処理対象無しの場合
                writeLog(Level.INFO_INT, "支払方法マスタは存在しませんでした。");
            }

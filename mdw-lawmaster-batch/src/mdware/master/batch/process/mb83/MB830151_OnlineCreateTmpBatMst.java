...
/**
 * <p>タイトル: MB83-01-51 ONLINE店別商品データ作成 - TMP BATマスタ作成処理</p>
...
public class MB830151_OnlineCreateTmpBatMst {
	...
	//バッチID & バッチ名
	private String BATCH_UID = "MB83-01-51";
	private static final String BATCH_NA  = "ONLINE店別商品データ作成 - ONLINE TMP BATマスタ作成処理";
...
//												{"TMP_BAT_TENBETU_SIIRESAKI",		"TMP BAT 店別仕入先マスタ"},
	 ...
	 * @param dataBase
	 */
	public MB830151_OnlineCreateTmpBatMst(DataBase db) {
		this.db = db;
		if (db == null) {
	 ...
	 * コンストラクタ
	 */
	public MB830151_OnlineCreateTmpBatMst() {
...
　	*/	
	...
	{	
		...
		try {
			...
			//開始ログ出力
			LogInfo = "ONLINE TMP BAT 系マスタ　作成処理を開始します";
			...
			//終了ログ出力
			LogInfo = "ONLINE TMP BAT 系マスタ　作成処理を終了します";
			writeLog(Level.INFO_INT,  LogInfo);	
...
　	*/	
	...
	{	
		...
		try {
				...
				sb.append("INSERT /*+ APPEND */ INTO TMP_BAT_SYOHIN_ONLINE NOLOGGING ");
				sb.append("( ");
				...
				//終了ログ出力
				LogInfo = "ONLINE TMP BAT 商品マスタを" + sqlCount + "件、作成しました。";
				writeLog(Level.INFO_INT,  LogInfo);
...
　	*/	
	...
	{	
		...
		try {
				...
				sb.append("INSERT /*+ APPEND */ INTO TMP_BAT_TENSYOHIN_REI_ONLINE NOLOGGING ");
				sb.append("( ");
				sb.append("       BUNRUI1_CD, ");
				sb.append("       SYOHIN_CD, ");
				sb.append("       TENPO_CD, ");
				...
				sb.append("  FROM R_TENSYOHIN_REIGAI R ");
				sb.append(" WHERE YUKO_DT > '" + DateChanger.addDate(today, -3) + "' ");
				...
				//終了ログ出力
				LogInfo = "ONLINE TMP BAT 店別商品例外マスタを" + sqlCount + "件、作成しました。";
				writeLog(Level.INFO_INT,  LogInfo);
	...
	public static void main(String[] args) {
		...
		MB830151_OnlineCreateTmpBatMst batch = new MB830151_OnlineCreateTmpBatMst();
		try {
			batch.execute();
		} catch (Exception e) {
			e.printStackTrace();

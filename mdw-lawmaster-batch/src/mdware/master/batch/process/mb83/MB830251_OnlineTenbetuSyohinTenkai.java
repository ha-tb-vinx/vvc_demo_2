...
/**
 * <p>タイトル: MB83-02-51 ONLINE店別商品データ作成 - 店別商品展開処理</p>
...
public class MB830251_OnlineTenbetuSyohinTenkai {
	...
	//バッチID & バッチ名
	private String BATCH_UID = "MB83-02-51";
	private static final String BATCH_NA  = "ONLINE店別商品データ作成 - ONLINE店別商品展開処理";
	 ...
	 * @param dataBase
	 */
	public MB830251_OnlineTenbetuSyohinTenkai(DataBase db) {
		this.db = db;
		if (db == null) {
...
//			this.db = new DataBase("rbsite_ora");
	 ...
	 * コンストラクタ
	 */
	public MB830251_OnlineTenbetuSyohinTenkai() {
...
　	*/	
	...
	public void execute() throws Exception {
		...
		try {
			...
			//STEP2：ONLINE TMP 店別商品データ一括削除処理
			truncateTmpM83TenbetuSyohin();
		 
			//STEP3：ONLINE TMP 店別商品データ作成処理
...
　	*/
	...
	{	
		...
		try {
			...
			//TRUNCATE処理
			DBUtil.truncateTable(db, "WK_MB83_TENBETU_SYOHIN_ONLINE");

			//削除ログ出力
			writeLog(Level.INFO_INT, "ONLINE TMP 店別商品データを削除しました");						
...
　	*/	
	...
	{	
		...
		try {
			...
			//開始ログ出力
			writeLog(Level.INFO_INT, "ONLINE TMP 店別商品データ　作成処理を開始します");
			...
			//結果ログ出力
			writeLog(Level.INFO_INT, "ONLINE TMP 店別商品データを" + sqlCount + "件、作成しました。");
			
			//終了ログ出力
			writeLog(Level.INFO_INT, "ONLINE TMP 店別商品データ　作成処理を終了します");	
...
　	*/	
	...
	{	
		...
		sb.append("   INTO WK_MB83_TENBETU_SYOHIN_ONLINE NOLOGGING ( ");											
		...
		sb.append("   FROM TMP_BAT_SYOHIN_ONLINE  S ");
		sb.append("  INNER JOIN");
		sb.append("        R_TANPINTEN_TORIATUKAI T ");
	...
	public static void main(String[] args) {
		...
		MB830251_OnlineTenbetuSyohinTenkai batch = new MB830251_OnlineTenbetuSyohinTenkai();
		try {
			batch.execute();
		} catch (Exception e) {
			e.printStackTrace();

...
/**
 * <p>タイトル: MB83-04-01 店別商品データ作成 - 店別例外反映処理</p>
 ...
 *  @version 1.1 #6678 (2022.10.27) VU.TD
 */
public class MB830401_TenbetuReigaiHanei {
	...
	//バッチID & バッチ名
	private String BATCH_UID = "MB83-04-01";
	private static final String BATCH_NA  = "店別商品データ作成 - 店別例外反映処理";
	 ...
	 * @param dataBase
	 */
	public MB830401_TenbetuReigaiHanei(DataBase db) {
		this.db = db;
		if (db == null) {
...
//			this.db = new DataBase("rbsite_ora");
	 ...
	 * コンストラクタ
	 */
	public MB830401_TenbetuReigaiHanei() {
...
　	*/
	private void truncateTenbetuSyohin() throws Exception
	{
		...
		try {
			...
			//TRUNCATE処理
			DBUtil.truncateTable(db, "DT_TENBETU_SYOHIN");
...
　	*/
	private void createTenbetuSyohin() throws Exception
	{
		...
		try {
			...
			//結果ログ出力
			LogInfo = "店別商品データを" + sqlCount + "件、作成しました。";
			writeLog(Level.INFO_INT, LogInfo);
			...
			//結果ログ出力
			LogInfo = "店別商品データを" + sqlCount + "件、作成しました。(店別例外から追加）";
			writeLog(Level.INFO_INT, LogInfo);
			...
			//結果ログ出力
			LogInfo = "店別商品データを" + sqlCount + "件、処理しました。(内部IF対象フラグの調整）";
			writeLog(Level.INFO_INT, LogInfo);

			//終了ログ出力
			LogInfo = "店別商品データ　作成処理を終了します";
			writeLog(Level.INFO_INT, LogInfo);
...
　	*/
	private String getInsertTenbetuSyohin() throws Exception
	{
		...
		sb.append(" INSERT /*+ APPEND */ INTO DT_TENBETU_SYOHIN NOLOGGING ");
		sb.append(" (");
		...
		// #6678 MOD 2022.10.26 VU.TD (S)
//		sb.append("        COALESCE(R.EOS_KB,       S.EOS_KB)       AS EOS_KB, ");			// EOS区分
		sb.append("        COALESCE(S.EOS_KB,       R.EOS_KB)       AS EOS_KB, ");
		// #6678 MOD 2022.10.26 VU.TD (E)
		...
		sb.append("           FROM WK_MB83_TENBETU_SYOHIN W ");
		...
		sb.append("        ) S ");
		sb.append("   LEFT JOIN ");
		sb.append("        TMP_BAT_TENSYOHIN_REIGAI R ");
...
　	*/
	private String getInsertTenbetuSyohinReigai() throws Exception
	{
		...
		sb.append(" INSERT INTO DT_TENBETU_SYOHIN ");
		sb.append(" (");
		sb.append("        TENPO_CD, ");													// 店舗コード
		sb.append("        BUNRUI1_CD, ");													// 分類１コード
		sb.append("        SYOHIN_CD, ");													// 商品コード
		...
		sb.append("        COALESCE(R.BAITANKA_VL,  S.BAITANKA_VL)  AS BAITANKA_VL, ");		// 売単価
		sb.append("        COALESCE(R.BUTURYU_KB,   S.BUTURYU_KB)   AS BUTURYU_KB,");		// 納品区分
		// #6678 MOD 2022.10.27 VU.TD (S)
		//sb.append("        COALESCE(R.EOS_KB,       S.EOS_KB)       AS EOS_KB, ");			// EOS区分
		sb.append("        COALESCE(S.EOS_KB,       R.EOS_KB)       AS EOS_KB, ");            // EOS区分
		// #6678 MOD 2022.10.27 VU.TD (E)
		sb.append("        S.NON_ACT_KB, ");												// NON-ACT区分
		sb.append("        S.TENPO_KB, ");													// 店舗区分
		sb.append("        CASE WHEN R.HENKO_DT > S.HENKO_DT ");							// 変更日
		sb.append("             THEN R.HENKO_DT ");											//
		...
		sb.append("   FROM TMP_BAT_TENSYOHIN_REIGAI R ");
		sb.append("  INNER JOIN ");
		sb.append("        WK_MB83_TENBETU_SYOHIN S ");
		...
		// #6620 ADD 2022.11.30 TUNG.LT (S)
		sb.append("    AND S.NAIBU_IF_FG   =  '" + mst010501_NaibuIFFgDictionary.TAISYO.getCode() + "'");
		// #6620 ADD 2022.11.30 TUNG.LT (E)
...
//		sb.append(" SET NAIBU_IF_FG = '" + mst010501_NaibuIFFgDictionary.TAISYOGAI.getCode() + "' ");
	...
	public static void main(String[] args) {
		...
		MB830401_TenbetuReigaiHanei batch = new MB830401_TenbetuReigaiHanei();
		try {
			batch.execute();
		} catch (Exception e) {
			e.printStackTrace();

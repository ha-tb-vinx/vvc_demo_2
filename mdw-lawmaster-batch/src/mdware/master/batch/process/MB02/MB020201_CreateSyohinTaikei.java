...
/**
 * <p>タイトル:指定日商品分類体系マスタ作成処理</p>
 * <p>説明:商品階層マスタに登録されている内容に基づき、指定日商品分類体系マスタ作成を行う。</p>
 * <p>著作権: Copyright (c) 2004</p>
 * <p>会社名: Vinculum Japan Corp.</p>
 * @version 1.00 2017.01.16 S.Takayama #1749 FIVIMART対応
 */
 
public class MB020201_CreateSyohinTaikei {
	 ...
	 * @param dataBase
	 */
	public MB020201_CreateSyohinTaikei(MasterDataBase db) {
		this.db = db;
		if (db == null) {
	 ...
	 * コンストラクタ
	 */
	public MB020201_CreateSyohinTaikei() {
	...
	public void execute() throws Exception {
		...
		try{
			...
			writeLog(Level.INFO_INT, "指定日指定日商品分類体系マスタの物理削除処理を開始します。");
			//物理削除する
			strSql.delete(0,strSql.length());
			strSql.append(" DELETE FROM ");
			strSql.append("   R_STI_SYOHIN_TAIKEI ");

			int delNum = db.executeUpdate(strSql.toString());
			
			writeLog(Level.INFO_INT, "指定日商品分類体系マスタを " + delNum + "件削除しました。");
			writeLog(Level.INFO_INT, "指定日商品分類体系マスタの物理削除処理を終了します。");

			writeLog(Level.INFO_INT, "指定日商品分類体系マスタの登録処理を開始します。");
			
			//商品分類階層マスタから指定日商品分類体系マスタを生成する。
			//StringBufferの内容をクリアする
			strSql.delete(0,strSql.length());
			
			strSql.append("INSERT INTO R_STI_SYOHIN_TAIKEI(");
			...
			writeLog(Level.INFO_INT, "指定日商品分類体系マスタの登録処理を終了します。");

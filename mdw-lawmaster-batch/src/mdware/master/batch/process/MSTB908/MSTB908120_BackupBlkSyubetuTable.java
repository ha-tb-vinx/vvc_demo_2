...
/**
 *
 * <p>タイトル: MSTB908120_BackupBlkSyubetuTable.java クラス</p>
 * <p>説明　: IF_BLK支払種別マスタ、IF_BLK特売種別マスタの内容を、ZEN_BLK支払種別マスタ、ZEN_BLK特売種別マスタに取込む。</p>
 * <p>著作権: Copyright (c) 2017</p>
 * <p>会社名: VINX</p>
 * @version 1.00 (2017.04.06) M.Son #2463 FIVImart様対応
 *
 */
public class MSTB908120_BackupBlkSyubetuTable {
	...
	// テーブル
	private static final String IF_BLK_PAYMENT = "IF_BLK_PAYMENT"; // IF_BLK支払種別マスタ
	private static final String ZEN_BLK_PAYMENT = "ZEN_BLK_PAYMENT"; // ZENBLK支払種別マスタ

	private static final String IF_BLK_DISCOUNT = "IF_BLK_DISCOUNT"; // IF_BLK特売種別マスタ
	 ...
	 * @param dataBase
	 */
	public MSTB908120_BackupBlkSyubetuTable(DataBase db) {
		this.db = db;
		if (db == null) {
	 ...
	 * コンストラクタ
	 */
	public MSTB908120_BackupBlkSyubetuTable() {
	...
	public void execute() throws Exception {
		...
		try {
			...
			// IF_BLK支払種別マスタのバックアップ（IF→BK_IF）
			writeLog(Level.INFO_INT, "IF_BLK支払種別マスタのバックアップ処理を開始します。");
			iRec = db.executeUpdate(getAllInsertSQL(IF_BLK_PAYMENT, ZEN_BLK_PAYMENT));
			writeLog(Level.INFO_INT, "ZENBLK支払種別マスタへ" + iRec + "件登録しました。");
			writeLog(Level.INFO_INT, "IF_BLK支払種別マスタのバックアップ処理を終了します。");
			...
			// IF_BLK特売種別マスタのバックアップ（IF→BK_IF）
			writeLog(Level.INFO_INT, "IF_BLK特売種別マスタのバックアップ処理を開始します。");
			iRec = db.executeUpdate(getAllInsertSQL(IF_BLK_DISCOUNT, ZEN_BLK_DISCOUNT));
			writeLog(Level.INFO_INT, "ZENBLK特売種別マスタへ" + iRec + "件登録しました。");
			writeLog(Level.INFO_INT, "IF_BLK特売種別マスタのバックアップ処理を終了します。");

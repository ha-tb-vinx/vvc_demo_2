...
/**
 * <p>タイトル:システム日付更新処理</p>
 * <p>説明:システムコントロールのシステム日付（ポータルオンライン日付）を更新する。</p>
...
public class MB010103_RenewDatePortalOnlineDt {
	 ...
	 * @param dataBase
	 */
	public MB010103_RenewDatePortalOnlineDt(MasterDataBase db) {
		this.db = db;
		if (db == null) {
	 ...
	 * コンストラクタ
	 */
	public MB010103_RenewDatePortalOnlineDt() {
	...
	public void execute() throws Exception {
		...
		try{
			...
			writeLog(Level.INFO_INT, "（ポータル）オンライン日付更新処理を開始します。【変更前："+ DateChanger.toSeparatorDate(mRec) +"】");
			...
			if(iRec == 0){
				...
				writeLog(Level.INFO_INT, "（ポータル）オンライン日付更新に成功しました。  【変更後："+ DateChanger.toSeparatorDate(DateChanger.addDate(mRec,SYORI_DAYS)) +"】");
			}

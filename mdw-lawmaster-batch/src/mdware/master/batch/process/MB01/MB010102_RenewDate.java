...
/**
 * <p>タイトル:システム日付更新処理</p>
 * <p>説明:システムコントロールのシステム日付（バッチ日付）を更新する。</p>
...
public class MB010102_RenewDate {
	 ...
	 * @param dataBase
	 */
	public MB010102_RenewDate(MasterDataBase db) {
		this.db = db;
		if (db == null) {
	...
	public void execute() throws Exception {
		...
		try{
			...
			ResultSet brs = db.executeQuery(strSql.toString());

			//バッチ日付を変数に格納する
			if(brs.next()){
				mRec = brs.getString("PARAMETER_TX");
			}
			db.closeResultSet(brs);
			
			writeLog(Level.INFO_INT, "（マスタ）バッチ日付更新処理を開始します。【変更前："+ DateChanger.toSeparatorDate(mRec) +"】");
			...
			if(iRec == 0){
				...
				writeLog(Level.INFO_INT, "（マスタ）バッチ日付更新に成功しました。  【変更後："+ DateChanger.toSeparatorDate(DateChanger.addDate(mRec,SYORI_DAYS)) +"】");
			}

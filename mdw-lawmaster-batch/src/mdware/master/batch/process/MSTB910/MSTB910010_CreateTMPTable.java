...
/**
 * <p> タイトル: MSTB910010_CreateTMPTableクラス</p>
 * <p> 説明: 緊急バッチ用マスタ作成処理</p>
 * <p> 著作権: Copyright (c) 2015 </p>
 * <p> 会社名: Vinculum Japan Corporation </p>
 * @author VINX
 * @Version 1.00 (2015.10.13) DUNG.NQ FIVIMART対応
 * @Version 1.01 (2016.05.06) M.Kanno #1306対応
 */

public class MSTB910010_CreateTMPTable {
	 ...
	 * @param dataBase
	 */
	public MSTB910010_CreateTMPTable(MasterDataBase db) {
		this.db = db;
		if (db == null) {
	 ...
	 * コンストラクタ
	 */
	public MSTB910010_CreateTMPTable() {
	...
	public void execute() throws Exception {
		...
		try {
			...
			for (Iterator ite = tableMap.entrySet().iterator(); ite.hasNext();) {
				...
				// テーブルの存在チェック
				if (table_fm.equals("R_SYOHIN")) {
					writeLog(Level.INFO_INT, table_fm + "から" + table_to 	+ "への複製を開始します。");
					dropTable(table_to);
					db.executeUpdate("CREATE TABLE TMP_R_EMG_SYOHIN AS SELECT * FROM R_SYOHIN ");
					writeLog(Level.INFO_INT, table_fm + "から" + table_to	+ "への複製を終了します。");
				} else if (table_fm.equals("R_SYOHIN_REIGAI")) {
					writeLog(Level.INFO_INT, table_fm + "から" + table_to	+ "への複製を開始します。");
					dropTable(table_to);
					db.executeUpdate("CREATE TABLE TMP_R_EMG_SYOHIN_REIGAI AS SELECT * FROM R_SYOHIN_REIGAI ");
					writeLog(Level.INFO_INT, table_fm + "から" + table_to	 + "への複製を終了します。");
				} else if (table_fm.equals("R_TENPO")) {
					writeLog(Level.INFO_INT, table_fm + "から" + table_to + "への複製を開始します。");
					dropTable(table_to);
					db.executeUpdate("CREATE TABLE TMP_R_EMG_TENPO AS SELECT * FROM R_TENPO ");
					writeLog(Level.INFO_INT, table_fm + "から" + table_to + "への複製を終了します。");
				} else if (table_fm.equals("R_TORIATUKAI")) {
					writeLog(Level.INFO_INT, table_fm + "から" + table_to + "への複製を開始します。");
					dropTable(table_to);
					db.executeUpdate("CREATE TABLE TMP_R_EMG_TORIATUKAI AS SELECT * FROM R_TORIATUKAI ");
					writeLog(Level.INFO_INT, table_fm + "から" + table_to + "への複製を終了します。");
				...
				//}
					...
					writeLog(Level.INFO_INT, table_fm + "から" + table_to	+ "への複製を開始します。");
					dropTable(table_to);
					db.executeUpdate("CREATE TABLE TMP_R_EMG_SYOHIN_ASN AS SELECT * FROM R_SYOHIN_ASN ");
					writeLog(Level.INFO_INT, table_fm + "から" + table_to	 + "への複製を終了します。");
				} else if (table_fm.equals("R_TAX_RATE")) {
					writeLog(Level.INFO_INT, table_fm + "から" + table_to	+ "への複製を開始します。");
					dropTable(table_to);
					db.executeUpdate("CREATE TABLE TMP_R_EMG_TAX_RATE AS SELECT * FROM R_TAX_RATE");
					writeLog(Level.INFO_INT, table_fm + "から" + table_to	 + "への複製を終了します。");
				}
	...
	private String selectCreateTableSql(String batchDt) {
		...
		strSql.append("    AND DELETE_FG = '" + mst000101_ConstDictionary.DELETE_FG_NOR + "' ");
		strSql.append("    AND CREATE_TABLE_KB = '" + mst010401_CreateTableKbDictionary.KK.getCode() + "' ");
	...
	private String updateCreateTableSql(String batchDt, String tableFrom)
		...
		strSql.append("  WHERE ");
		strSql.append("        CREATE_TABLE_KB   = '" + mst010401_CreateTableKbDictionary.KK.getCode() + "' ");

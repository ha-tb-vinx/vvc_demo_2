...
/**
 * <p>タイトル:新店用単品店取扱マスタ作成処理</p>
 * <p>説明:新店取扱登録条件に設定されている内容に基づき、単品店取扱マスタに対して新店用のデータ作成を行う。</p>
 * <p>著作権: Copyright (c) 2009</p>
 * <p>会社名: Vinculum Japan Corp.</p>
 * @version 1.00 2009/02/13 マミーマート様向け初版作成 T.Mori
 * @version 1.01 2011/01/12 Y.Imai Mod マスタコピー時不稼働→稼働処理 AM00071
 * @author T.Mori
 */
 
public class MBA00101_CreateTanpintenToriatukaiMst {
	 ...
	 * @param dataBase
	 */
	public MBA00101_CreateTanpintenToriatukaiMst(MasterDataBase db) {
		this.db = db;
		if (db == null) {
	 ...
	 * コンストラクタ
	 */
	public MBA00101_CreateTanpintenToriatukaiMst() {
	...
	public void execute() throws Exception {
		...
		try{
			...
			while (rsSet.next()){
				...
				writeLog(Level.INFO_INT, "単品店取扱マスタデータ確認処理を開始します。");
				
				//単品店取扱マスタの登録対象データ件数を取得する。
				strSql.delete(0,strSql.length());
				strSql.append("SELECT ");
				strSql.append("  COUNT(*) AS COUNT ");
				strSql.append("FROM ");
				strSql.append("  R_TANPINTEN_TORIATUKAI ");
				strSql.append("WHERE ");
				...
				if ( iRec > 0 ){
					...
					writeLog(Level.INFO_INT, "単品店取扱マスタの物理削除処理を開始します。");
					//登録データを物理削除する
					strSql.delete(0,strSql.length());
					...
					writeLog(Level.INFO_INT, "単品店取扱マスタを " + delNum + "件削除しました。");
					writeLog(Level.INFO_INT, "単品店取扱マスタの物理削除処理を終了します。");

				}

				writeLog(Level.INFO_INT, "単品店取扱マスタの登録処理を開始します。");
				
				//登録する
				strSql.delete(0,strSql.length());
				strSql.append("INSERT INTO R_TANPINTEN_TORIATUKAI(");
				strSql.append("  BUNRUI1_CD,");
				strSql.append("  SYOHIN_CD,");
				strSql.append("  TENPO_CD,");
				strSql.append("  DONYU_ST_DT,");
				strSql.append("  DONYU_ED_DT,");
				strSql.append("  NON_ACT_KB,");
				strSql.append("  NON_ACT_SOSHIN_DT,");
				strSql.append("  SAISHIN_HACYU_DT,");
				strSql.append("  SAISHIN_SHIIRE_DT,");
				strSql.append("  SAISHIN_URIAGE_DT,");
				strSql.append("  HASEIMOTO_KB,");
				strSql.append("  TANAWARI_PATERN,");
				strSql.append("  JUKI_NB,");
				strSql.append("  TANA_NB,");
				strSql.append("  FACE_QT,");
				strSql.append("  MIN_TINRETU_QT,");
				strSql.append("  MAX_TINRETU_QT,");
				strSql.append("  BASE_ZAIKO_NISU_QT,");
				...
				strSql.append("	   DONYU_ED_DT,");
//2011.01.12 Y.Imai Mod マスタコピー時不稼働→稼働処理 AM00071 Start
//				strSql.append("    NON_ACT_KB,");
				strSql.append("    '" + mst007601_NonActKbDictionary.ACT.getCode() + "', ");
//				strSql.append("    NON_ACT_SOSHIN_DT,");
				strSql.append("    NULL,");
//2011.01.12 Y.Imai Mod マスタコピー時不稼働→稼働処理 AM00071 End
				strSql.append("    SAISHIN_HACYU_DT,");
//2011.01.12 Y.Imai Mod マスタコピー時不稼働→稼働処理 AM00071 Start
//				strSql.append("    SAISHIN_SHIIRE_DT,");
				strSql.append("    '" + DateChanger.addDate(batchDate, 1) + "', ");
//2011.01.12 Y.Imai Mod マスタコピー時不稼働→稼働処理 AM00071 End
				strSql.append("    SAISHIN_URIAGE_DT,");
				strSql.append("    HASEIMOTO_KB,");
				strSql.append("    TANAWARI_PATERN,");
				...
				writeLog(Level.INFO_INT, "単品店取扱マスタの登録処理を終了します。");

				writeLog(Level.INFO_INT, "登録件数確認処理を開始します。");
				
				iRec = 0;
				
				//単品店取扱マスタに登録した件数を取得する。
				strSql.delete(0,strSql.length());
				strSql.append("SELECT ");
				strSql.append("  BUNRUI1_CD, ");
				strSql.append("  TENPO_CD, ");

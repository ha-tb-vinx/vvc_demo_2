/**
 * <p>タイトル: 新ＭＤシステム（マスター管理）品種一括送信画面の検索結果レコード格納用クラス</p>
 * <p>説明: 新ＭＤシステムで使用する品種一括送信の検索結果レコード格納用クラス(DmCreatorにより自動生成)</p>
 ...
 * @version 1.0(2005/05/10)初版作成
 */
public class mst760301_HinsyuSendListBean
{
	...
	public static final int HINSYU_KANJI_RN_MAX_LENGTH = 20;		//品種名称の長さ
	...
	private String hinsyu_kanji_rn = null;			//品種名称
	...
	/**
	 * mst760301_HinsyuSendListBeanを１件のみ抽出したい時に使用する
	 */
	public static mst760301_HinsyuSendListBean getmst760301_HinsyuSendListBean(DataHolder dataHolder)
	{
		mst760301_HinsyuSendListBean bean = null;
		BeanHolder beanHolder = new BeanHolder( new mst760301_HinsyuSendListDM() );
		try
		{
			Iterator ite = beanHolder.getPageBeanListFromDataHolder( dataHolder ).iterator();
			//１件以上存在する時はまず保存する
			if( ite.hasNext() )
				bean = (mst760301_HinsyuSendListBean )ite.next();
			//２件以上存在する時はＮＵＬＬにする
			if( ite.hasNext() )
				bean = null;
		}
	 ...
	 * @return mst760301_HinsyuSendListBean コピー後のクラス
	 */
	public mst760301_HinsyuSendListBean createClone()
	{
	...
	public boolean equals( Object o )
	{
		...
		if( !( o instanceof mst760301_HinsyuSendListBean ) ) return false;

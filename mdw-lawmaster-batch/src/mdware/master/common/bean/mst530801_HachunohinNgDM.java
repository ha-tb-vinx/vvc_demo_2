/**
 * <p>タイトル: 新ＭＤシステム（マスター管理）店発注納品不可日マスタ（生鮮）のDMクラス</p>
 * <p>説明: 新ＭＤシステムで使用する店発注納品不可日マスタ（生鮮）のDMクラス(DmCreatorにより自動生成)</p>
 ...
 * @version 1.0(2005/03/18)初版作成
 */
public class mst530801_HachunohinNgDM extends DataModule
{
	private DBStringConvert conv = DBStringConvert.getDBStringConvert( getDatabaseProductName() );
	/**
	 * コンストラクタ
	 */
	public mst530801_HachunohinNgDM()
	{
...
//		******** ＤＢ Ver3.6修正箇所 *****************************************
		bean.setInsertUserTs( rest.getString("insert_user_id") );
...
//		******** ＤＢ Ver3.6修正箇所 *****************************************
		...
		sb.append(", ");
		sb.append("delete_fg ");
		sb.append("from R_HACHUNOHIN_NG ");
		...
		{
			sb.append(whereStr);
			sb.append("ymd_dt >= '" + conv.convertWhereString( (String)map.get("ymd_dt_bef") ) + "'");
			whereStr = andStr;
		}
		if( map.get("ymd_dt_aft") != null && ((String)map.get("ymd_dt_aft")).trim().length() > 0 )
		{
			sb.append(whereStr);
			sb.append("ymd_dt <= '" + conv.convertWhereString( (String)map.get("ymd_dt_aft") ) + "'");
			whereStr = andStr;
		}
		if( map.get("ymd_dt") != null && ((String)map.get("ymd_dt")).trim().length() > 0 )
		{
			sb.append(whereStr);
			sb.append("ymd_dt = '" + conv.convertWhereString( (String)map.get("ymd_dt") ) + "'");
			whereStr = andStr;
		}
		if( map.get("ymd_dt_like") != null && ((String)map.get("ymd_dt_like")).trim().length() > 0 )
		{
			sb.append(whereStr);
			sb.append("ymd_dt like '%" + conv.convertWhereString( (String)map.get("ymd_dt_like") ) + "%'");
			whereStr = andStr;
		}
		if( map.get("ymd_dt_bef_like") != null && ((String)map.get("ymd_dt_bef_like")).trim().length() > 0 )
		{
			sb.append(whereStr);
			sb.append("ymd_dt like '%" + conv.convertWhereString( (String)map.get("ymd_dt_bef_like") ) + "'");
			whereStr = andStr;
		}
		if( map.get("ymd_dt_aft_like") != null && ((String)map.get("ymd_dt_aft_like")).trim().length() > 0 )
		{
			sb.append(whereStr);
			sb.append("ymd_dt like '" + conv.convertWhereString( (String)map.get("ymd_dt_aft_like") ) + "%'");
			whereStr = andStr;
		}
		if( map.get("ymd_dt_in") != null && ((String)map.get("ymd_dt_in")).trim().length() > 0 )
		{
			sb.append(whereStr);
			sb.append("ymd_dt in ( '" + replaceAll(conv.convertWhereString( (String)map.get("ymd_dt_in") ),",","','") + "' )");
			whereStr = andStr;
		}
		if( map.get("ymd_dt_not_in") != null && ((String)map.get("ymd_dt_not_in")).trim().length() > 0 )
		{
			sb.append(whereStr);
			sb.append("ymd_dt not in ( '" + replaceAll(conv.convertWhereString( (String)map.get("ymd_dt_not_in") ),",","','") + "' )");
			whereStr = andStr;
		}
...
//		{
//			sb.append(whereStr);
//			sb.append("hachu_ng_fg >= '" + conv.convertWhereString( (String)map.get("hachu_ng_fg_bef") ) + "'");
//			whereStr = andStr;
//		}
//		if( map.get("hachu_ng_fg_aft") != null && ((String)map.get("hachu_ng_fg_aft")).trim().length() > 0 )
//		{
//			sb.append(whereStr);
//			sb.append("hachu_ng_fg <= '" + conv.convertWhereString( (String)map.get("hachu_ng_fg_aft") ) + "'");
//			whereStr = andStr;
//		}
//		if( map.get("hachu_ng_fg") != null && ((String)map.get("hachu_ng_fg")).trim().length() > 0 )
//		{
//			sb.append(whereStr);
//			sb.append("hachu_ng_fg = '" + conv.convertWhereString( (String)map.get("hachu_ng_fg") ) + "'");
//			whereStr = andStr;
//		}
//		if( map.get("hachu_ng_fg_like") != null && ((String)map.get("hachu_ng_fg_like")).trim().length() > 0 )
//		{
//			sb.append(whereStr);
//			sb.append("hachu_ng_fg like '%" + conv.convertWhereString( (String)map.get("hachu_ng_fg_like") ) + "%'");
//			whereStr = andStr;
//		}
//		if( map.get("hachu_ng_fg_bef_like") != null && ((String)map.get("hachu_ng_fg_bef_like")).trim().length() > 0 )
//		{
//			sb.append(whereStr);
//			sb.append("hachu_ng_fg like '%" + conv.convertWhereString( (String)map.get("hachu_ng_fg_bef_like") ) + "'");
//			whereStr = andStr;
//		}
//		if( map.get("hachu_ng_fg_aft_like") != null && ((String)map.get("hachu_ng_fg_aft_like")).trim().length() > 0 )
//		{
//			sb.append(whereStr);
//			sb.append("hachu_ng_fg like '" + conv.convertWhereString( (String)map.get("hachu_ng_fg_aft_like") ) + "%'");
//			whereStr = andStr;
//		}
//		if( map.get("hachu_ng_fg_in") != null && ((String)map.get("hachu_ng_fg_in")).trim().length() > 0 )
//		{
//			sb.append(whereStr);
//			sb.append("hachu_ng_fg in ( '" + replaceAll(conv.convertWhereString( (String)map.get("hachu_ng_fg_in") ),",","','") + "' )");
//			whereStr = andStr;
//		}
//		if( map.get("hachu_ng_fg_not_in") != null && ((String)map.get("hachu_ng_fg_not_in")).trim().length() > 0 )
//		{
//			sb.append(whereStr);
//			sb.append("hachu_ng_fg not in ( '" + replaceAll(conv.convertWhereString( (String)map.get("hachu_ng_fg_not_in") ),",","','") + "' )");
//			whereStr = andStr;
//		}
		...
		{
			sb.append(whereStr);
			sb.append("nohin_ng_fg >= '" + conv.convertWhereString( (String)map.get("nohin_ng_fg_bef") ) + "'");
			whereStr = andStr;
		}
		if( map.get("nohin_ng_fg_aft") != null && ((String)map.get("nohin_ng_fg_aft")).trim().length() > 0 )
		{
			sb.append(whereStr);
			sb.append("nohin_ng_fg <= '" + conv.convertWhereString( (String)map.get("nohin_ng_fg_aft") ) + "'");
			whereStr = andStr;
		}
		if( map.get("nohin_ng_fg") != null && ((String)map.get("nohin_ng_fg")).trim().length() > 0 )
		{
			sb.append(whereStr);
			sb.append("nohin_ng_fg = '" + conv.convertWhereString( (String)map.get("nohin_ng_fg") ) + "'");
			whereStr = andStr;
		}
		if( map.get("nohin_ng_fg_like") != null && ((String)map.get("nohin_ng_fg_like")).trim().length() > 0 )
		{
			sb.append(whereStr);
			sb.append("nohin_ng_fg like '%" + conv.convertWhereString( (String)map.get("nohin_ng_fg_like") ) + "%'");
			whereStr = andStr;
		}
		if( map.get("nohin_ng_fg_bef_like") != null && ((String)map.get("nohin_ng_fg_bef_like")).trim().length() > 0 )
		{
			sb.append(whereStr);
			sb.append("nohin_ng_fg like '%" + conv.convertWhereString( (String)map.get("nohin_ng_fg_bef_like") ) + "'");
			whereStr = andStr;
		}
		if( map.get("nohin_ng_fg_aft_like") != null && ((String)map.get("nohin_ng_fg_aft_like")).trim().length() > 0 )
		{
			sb.append(whereStr);
			sb.append("nohin_ng_fg like '" + conv.convertWhereString( (String)map.get("nohin_ng_fg_aft_like") ) + "%'");
			whereStr = andStr;
		}
		if( map.get("nohin_ng_fg_in") != null && ((String)map.get("nohin_ng_fg_in")).trim().length() > 0 )
		{
			sb.append(whereStr);
			sb.append("nohin_ng_fg in ( '" + replaceAll(conv.convertWhereString( (String)map.get("nohin_ng_fg_in") ),",","','") + "' )");
			whereStr = andStr;
		}
		if( map.get("nohin_ng_fg_not_in") != null && ((String)map.get("nohin_ng_fg_not_in")).trim().length() > 0 )
		{
			sb.append(whereStr);
			sb.append("nohin_ng_fg not in ( '" + replaceAll(conv.convertWhereString( (String)map.get("nohin_ng_fg_not_in") ),",","','") + "' )");
			whereStr = andStr;
		}
...
//		DBStringConvert conv = DBStringConvert.getDBStringConvert( getDatabaseProductName() );
		mst530801_HachunohinNgBean bean = (mst530801_HachunohinNgBean)beanMst;
		StringBuffer sb = new StringBuffer();

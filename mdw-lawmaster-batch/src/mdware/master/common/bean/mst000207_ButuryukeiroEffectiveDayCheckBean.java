/**
 * <P>タイトル : 物流経路用有効日チェッククラス</P>
 * <P>説明 : 物流経路用有効日チェッククラス</P>
...
public class mst000207_ButuryukeiroEffectiveDayCheckBean {
	/**
	 * 有効データの存在チェック
	 * <br>
	 ...
	 * @param		String	sihaiKb		: 仕配区分
	 * @param		String	sihaiCd		: 仕配コード
	 * @param		String	syohinCd	: 商品コード
	 * @param		String	buturyuKb	: 物流区分
	 ...
	 */
	public static String getYukoDtCheck(String tableNm, String yukoDt, String kanriKb, String kanriCd, String sihaiKb,
											String sihaiCd, String syohinCd, String buturyuKb, String tenpoCd, Object Keepbeen,
												mst000101_DbmsBean db, String addValue) throws Exception,SQLException{
		//メッセージ取得
		...
		if(syorikb.equals(mst000101_ConstDictionary.PROCESS_INSERT)){
			if(!isYukoDtCheckInsert(db, kanriKb, kanriCd, sihaiKb, sihaiCd, syohinCd, buturyuKb, tenpoCd, yukoDt)){
				ErrorFlg = "EDEL";
			} else {
//				ErrorFlg = mst000101_ConstDictionary.UPDATE_PROCESS_INSERT;
		...
		if (syorikb.equals(mst000101_ConstDictionary.PROCESS_UPDATE)) {
			errorChk = isYukoDtCheckUpdete(db, map, tableNm, kanriKb, kanriCd, sihaiKb, sihaiCd, syohinCd, buturyuKb, tenpoCd,  yukoDt ,addValue);
...
//				ErrorFlg = errorChk;
		...
		if(syorikb.equals(mst000101_ConstDictionary.PROCESS_DELETE)){
			errorChk = isYukoDtCheckDelete(db, map, tableNm, kanriKb, kanriCd, sihaiKb, sihaiCd, syohinCd, buturyuKb, tenpoCd, yukoDt, addValue);
			if(errorChk.equals(mst000101_ConstDictionary.UPDATE_PROCESS_INSERT)
				 || errorChk.equals(mst000101_ConstDictionary.UPDATE_PROCESS_UPDATE)
...
//		param[0]         = new String(ErrorFlg);
	 ...
	 * @return boolean
	 */
	public static boolean isYukoDtCheckInsert(mst000101_DbmsBean db, String kanriKb, String kanriCd, String sihaiKb, String sihaiCd,
												String syohinCd, String buturyuKb, String tenpoCd, String yukoDt) throws SQLException {
		...
		String MasterDT = RMSTDATEUtil.getMstDateDt();
		//過去データ取得
		ResultSet resPst = db.executeQuery(selYukoDtPast("R_BUTURYUKEIRO", kanriKb, kanriCd, sihaiKb, sihaiCd, syohinCd,
											buturyuKb, tenpoCd, yukoDt));
		...
		if (!existsFg) {
			//未来データ取得
			ResultSet resFtr = db.executeQuery(selYukoDtFuture("R_BUTURYUKEIRO", kanriKb, kanriCd, sihaiKb, sihaiCd, syohinCd,
												buturyuKb, tenpoCd, yukoDt));
	...
	public static String isYukoDtCheckUpdete(mst000101_DbmsBean db, Map msg, String tableNm, String kanriKb, String kanriCd,
							String sihaiKb, String sihaiCd, String syohinCd, String buturyuKb, String tenpoCd, String yukoDt,
							String addValue) throws SQLException {
		...
		//過去データ取得
		ResultSet resPst = db.executeQuery(selYukoDtPast(tableNm, kanriKb, kanriCd, sihaiKb, sihaiCd, syohinCd,
															buturyuKb, tenpoCd, yukoDt));
		...
		if (resPst.next()) {
			...
			if (tmp_del.equals(mst000801_DelFlagDictionary.INAI.getCode())) {
				...
				if (tmp_yuko.equals(yukoDt)) {
					...
					UpdateProcessFlg = mst000101_ConstDictionary.UPDATE_PROCESS_INSERT;
					//■未来データは2件まで有効
					ResultSet rsCnt = db.executeQuery(getFtrDateCnt(kanriKb, kanriCd, sihaiKb, sihaiCd, syohinCd, buturyuKb, tenpoCd, addMasterDT));
					if (rsCnt.next()) {
		...
		if (existsFg && updateFg) {
			//未来データ取得
			ResultSet resFtr = db.executeQuery(selYukoDtFuture(tableNm, kanriKb, kanriCd, sihaiKb, sihaiCd, syohinCd, buturyuKb, tenpoCd, yukoDt));
	...
	public static String isYukoDtCheckDelete(mst000101_DbmsBean db, Map msg, String tableNm, String kanriKb, String kanriCd,
						String sihaiKb, String sihaiCd, String syohinCd, String buturyuKb, String tenpoCd, String yukoDt, String addValue) throws SQLException {
		boolean existsFg = false; //データが存在するか
		...
		//過去データ取得
		ResultSet resPst = db.executeQuery(selYukoDtPast(tableNm, kanriKb, kanriCd, sihaiKb, sihaiCd, syohinCd, buturyuKb, tenpoCd, yukoDt));
...
//				existsFg = false;
	 ...
	 * @throws
	 */
	public static String selYukoDtPast(String tableNm, String kanriKb, String kanriCd, String sihaiKb, String sihaiCd,
											String syohinCd, String buturyuKb, String tenpoCd, String yukoDt) throws SQLException{
		StringBuffer sql = new StringBuffer();
		...
		sql.append(" AND ");
		sql.append("   SIHAI_KB = '" + sihaiKb + "'");
		sql.append(" AND ");
		sql.append("   SIHAI_CD = '" + sihaiCd + "'");
		sql.append(" AND ");
		sql.append("   SYOHIN_CD = '" + syohinCd + "'");
		sql.append(" AND ");
		sql.append("   TENPO_CD = '"+ tenpoCd +"'");
		sql.append(" AND ");
		sql.append("   BUTURYU_KB = '"+ buturyuKb + "'");
		//有効日の当日までの直近データ取得
		...
		sql.append("      AND   SIHAI_KB = mst.SIHAI_KB");
		sql.append("      AND   SIHAI_CD = mst.SIHAI_CD");
		sql.append("      AND   SYOHIN_CD = mst.SYOHIN_CD");
		sql.append("      AND   TENPO_CD = mst.TENPO_CD");
		sql.append("      AND   BUTURYU_KB = mst.BUTURYU_KB");
	 ...
	 * @throws
	 */
	public static String selYukoDtFuture(String tableNm, String kanriKb, String kanriCd, String sihaiKb, String sihaiCd,
											String syohinCd, String buturyuKb, String tenpoCd, String yukoDt) throws SQLException {
	 ...
	 * @throws
	 */
	public static String getFtrDateCnt(String kanriKb, String kanriCd, String sihaiKb, String sihaiCd, String syohinCd,
											String buturyu_kb, String tenpoCd, String MasterDT) throws SQLException {
		...
		sql.append(" FROM ");
		sql.append("   R_BUTURYUKEIRO ");
		sql.append(" WHERE KANRI_KB = '"+ kanriKb +"'");
		sql.append(" AND   KANRI_CD = '"+ kanriCd +"'");
		sql.append(" AND   SIHAI_KB = '" + sihaiKb +"'");
		sql.append(" AND   SIHAI_CD = '" + sihaiCd + "'");

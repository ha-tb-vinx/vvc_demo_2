/**
 * <P>タイトル : 発注納品基準日用有効日チェッククラス</P>
 * <P>説明 : 発注納品基準日用有効日チェッククラス</P>
...
public class mst000205_HachunohinkijunbiEffectiveDayCheckBean {
	/**
	 * 有効データの存在チェック
	 * <br>
	 ...
	 * @param		String	siiresakiCd	: 仕入先コード
	 * @param		String	hachuhohoKb	: 発注方法区分
	 * @param		Object	Keepbeen	: 保持しているオブジェクト
	 * @param		String	addValue	: 暦日加算値
	 * @param		String	syorikb		: 処理状況
	 * @return		String	ErrorFlg	: エラーフラグ
	 */
	public static String getYukoDtCheck(String tableNm, String yukoDt, String kanriKb, String kanriCd, String siiresakiCd,
			String hachuhohoKb, Object Keepbeen, mst000101_DbmsBean db, String addValue) throws Exception,SQLException{
		//メッセージ取得
...
//		ErrorFlg = mst000101_ConstDictionary.ERR_CHK_NORMAL;
		...
		if(syorikb.equals(mst000101_ConstDictionary.PROCESS_INSERT)){
			if(!isYukoDtCheckInsert(db, kanriKb, kanriCd, siiresakiCd, hachuhohoKb, yukoDt)){
//				ErrorFlg = "EDEL";
				ErrorFlg = mst000101_ConstDictionary.ERR_CHK_ERROR;
				ErrorMessage = "指定されたデータは既に存在しています。";
			} else {
		...
		if (syorikb.equals(mst000101_ConstDictionary.PROCESS_UPDATE)) {
			errorChk = isYukoDtCheckUpdete(db, map, tableNm, kanriKb, kanriCd, siiresakiCd, hachuhohoKb, yukoDt ,addValue);
...
//				ErrorFlg = errorChk;
		...
		if(syorikb.equals(mst000101_ConstDictionary.PROCESS_DELETE)){
			...
			errorChk = isYukoDtCheckDelete(db, map, tableNm, kanriKb, kanriCd, siiresakiCd, hachuhohoKb, yukoDt, addValue);
...
//		param[0]         = new String(ErrorFlg);
	...
	/**
	 * 新規登録時の発注納品基準日マスタチェック
	 * @return boolean
	 */
	public static boolean isYukoDtCheckInsert(mst000101_DbmsBean db, String kanriKb, String kanriCd, String siiresakiCd,
													String hachuhohoKb, String yukoDt) throws SQLException {
		...
		//過去データ取得
		ResultSet resPst = db.executeQuery(selYukoDtPast("R_HACHUNOHINKIJUNBI", kanriKb, kanriCd, siiresakiCd, hachuhohoKb, yukoDt));
		...
		if (!existsFg) {
			//未来データ取得
			ResultSet resFtr = db.executeQuery(selYukoDtFuture("R_HACHUNOHINKIJUNBI", kanriKb, kanriCd, siiresakiCd, hachuhohoKb, yukoDt));
	...
	public static String isYukoDtCheckUpdete(mst000101_DbmsBean db, Map msg, String tableNm, String kanriKb, String kanriCd,
							String siiresakiCd, String hachuhohoKb, String yukoDt, String addValue) throws SQLException {
		...
		//過去データ取得
		ResultSet resPst = db.executeQuery(selYukoDtPast(tableNm, kanriKb, kanriCd, siiresakiCd, hachuhohoKb, yukoDt));
		...
		if (resPst.next()) {
			...
			if (tmp_del.equals(mst000801_DelFlagDictionary.INAI.getCode())) {
				...
				if (tmp_yuko.equals(yukoDt)) {
					...
					//■未来データは2件まで有効
					ResultSet rsCnt = db.executeQuery(getFtrDateCnt(kanriKb, kanriCd, siiresakiCd, hachuhohoKb, addMasterDT));
					if (rsCnt.next()) {
...
//							UpdateProcessFlg = "EFTR";
							UpdateProcessFlg = "将来データが2件、既に存在しています。";
						}
					}
				}
			} else {
		...
		if (existsFg && updateFg) {
			//未来データ取得
			ResultSet resFtr = db.executeQuery(selYukoDtFuture(tableNm, kanriKb, kanriCd, siiresakiCd, hachuhohoKb, yukoDt));
...
//				existsFg = false;
	...
	/**
	 * 発注納品基準日マスタチェック用 (過去データ)
	 * @throws
	 */
	public static String selYukoDtPast(String tableNm, String kanriKb, String kanriCd, String siiresakiCd, String hachuhohoKb,
		...
		sql.append(" AND ");
		sql.append("   SIIRESAKI_CD = '" + siiresakiCd + "'");
		sql.append(" AND ");
		sql.append("   HACHU_HOHO_KB = '" + hachuhohoKb + "'");
		//有効日の当日までの直近データ取得
		...
		sql.append("      AND   SIIRESAKI_CD = mst.SIIRESAKI_CD");
		sql.append("      AND   HACHU_HOHO_KB = mst.HACHU_HOHO_KB");
	...
	/**
	 * 発注納品基準日マスタチェック用 (未来データ)
	 * @throws
	 */
	public static String selYukoDtFuture(String tableNm, String kanriKb, String kanriCd, String siiresakiCd, String hachuhohoKb,
											String yukoDt) throws SQLException {
	 ...
	 * @throws
	 */
	public static String getFtrDateCnt(String kanriKb, String kanriCd, String siiresakiCd, String hachuhohoKb,
		...
		sql.append(" FROM ");
		sql.append("   R_HACHUNOHINKIJUNBI ");
		sql.append(" WHERE KANRI_KB = '"+ kanriKb +"'");
		sql.append(" AND   KANRI_CD = '"+ kanriCd +"'");
		sql.append(" AND   SIIRESAKI_CD = '"+ siiresakiCd + "'");
		sql.append(" AND   HACHU_HOHO_KB = '"+ hachuhohoKb + "'");

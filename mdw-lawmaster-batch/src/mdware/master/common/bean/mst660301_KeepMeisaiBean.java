/**
 * <P>タイトル : 新ＭＤシステム　マスターメンテナンス  mst660101用単品自動採番枠登録の画面項目格納用クラス</P>
 * <P>説明 : 新ＭＤシステムで使用するmst660101用単品自動採番枠登録の画面項目格納用クラス</P>
...
/**
 * <P>タイトル : 新ＭＤシステム　マスターメンテナンス  mst660101用単品自動採番枠登録の画面項目(一覧)格納用クラス</P>
 * <P>説明 : 新ＭＤシステムで使用するmst660101用単品自動採番枠登録の画面項目格納用クラス</P>
 *  <P>著作権: Copyright (c) 2005</p>								
 *  <P>会社名: Vinculum Japan Corp.</P>								
 * @author Sirius T.Kiiuchi
 ...
 * @see なし								
 */
public class mst660301_KeepMeisaiBean
{
	private static final StcLog stcLog = StcLog.getInstance();

	private ArrayList Meisai	= new ArrayList();	// 一覧の明細
	private ArrayList MeisaiAll	= new ArrayList();	// 一覧の明細
	...
	// MeisaiAllに対するセッターとゲッターの集合
	public boolean setMeisaiAll(ArrayList MeisaiAll) {
		this.MeisaiAll = MeisaiAll;
		return true;
	}
	
	public ArrayList getMeisaiAll() {
		return this.MeisaiAll;
	}

/**
 * <P>タイトル : 新ＭＤシステム　マスターメンテナンス  mst810101_Keiryoki用計量器マスタのレコード格納用クラス</P>
 * <P>説明 : 新ＭＤシステムで使用するmst810101_Keiryoki用計量器マスタのレコード格納用クラス(DmCreatorにより自動生成)</P>
 ...
 * @see なし
 */
public class mst810101_KeiryokiBean
{
	 ...
	 */
	public static mst810101_KeiryokiBean getRKeiryokiBean(DataHolder dataHolder)
	{
		mst810101_KeiryokiBean bean = null;
		BeanHolder beanHolder = new BeanHolder( new mst810101_KeiryokiDM() );
		try
		{
			Iterator ite = beanHolder.getPageBeanListFromDataHolder( dataHolder ).iterator();
			//１件以上存在する時はまず保存する
			if( ite.hasNext() )
				bean = (mst810101_KeiryokiBean )ite.next();
			//２件以上存在する時はＮＵＬＬにする
			if( ite.hasNext() )
				bean = null;
		}
	 ...
	 */
	public mst810101_KeiryokiBean createClone()
	{
	...
	public boolean equals( Object o )
	{
		...
		if( !( o instanceof mst810101_KeiryokiBean ) ) return false;

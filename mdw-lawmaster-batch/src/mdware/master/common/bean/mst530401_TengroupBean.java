 ...
 * @version 1.0(2005/03/18)初版作成
 */
public class mst530401_TengroupBean
{
	private static final StcLog stcLog = StcLog.getInstance();

	public static final int L_GYOSYU_CD_MAX_LENGTH = 4;//大業種 (FK)の長さ
	public static final int YOTO_KB_MAX_LENGTH = 1;//用途区分 (FK)の長さ
	public static final int GROUPNO_CD_MAX_LENGTH = 2;//グルーピングNO (FK)の長さ
	public static final int TENPO_CD_MAX_LENGTH = 6;//店舗コード (FK)の長さ
	public static final int RANK_NB_MAX_LENGTH = 3;//順位の長さ
	...
	private String rank_nb = null;	//順位
	...
	/**
	 * RTENGROUPBeanを１件のみ抽出したい時に使用する
	 */
	public static mst530401_TengroupBean getRTENGROUPBean(DataHolder dataHolder)
	{
		mst530401_TengroupBean bean = null;
		BeanHolder beanHolder = new BeanHolder( new mst530401_TengroupDM() );
		try
		{
			Iterator ite = beanHolder.getPageBeanListFromDataHolder( dataHolder ).iterator();
			//１件以上存在する時はまず保存する
			if( ite.hasNext() )
				bean = (mst530401_TengroupBean )ite.next();
			//２件以上存在する時はＮＵＬＬにする
			if( ite.hasNext() )
				bean = null;
		}
	...
	public String getRankNb()
	{
		return cutString(this.rank_nb,RANK_NB_MAX_LENGTH);
	}
	public String getRankNbString()
	{
		return "'" + cutString(this.rank_nb,RANK_NB_MAX_LENGTH) + "'";
	}
	public String getRankNbHTMLString()
	{
		return HTMLStringUtil.convert(cutString(this.rank_nb,RANK_NB_MAX_LENGTH));
	}
	 ...
	 * @return RTENGROUPBean コピー後のクラス
	 */
	public mst530401_TengroupBean createClone()
	{
	...
	public boolean equals( Object o )
	{
		...
		if( !( o instanceof mst530401_TengroupBean ) ) return false;

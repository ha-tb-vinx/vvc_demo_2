 ...
 * @version 1.0(2005/03/20)初版作成
 */
public class mst300201_TanpinTenToriatukaiBean
{
...
// 	↑↑仕様変更（2005.08.01）↑↑
//      ↓↓2006.06.21 fanglh カスタマイズ修正↓↓	
	public static final int SYORI_FLG_MAX_LENGTH = 6;			// 処理区分の長さ
	...
	public static final int SAISHIN_ERIAGE_DT_MAX_LENGTH = 8;	// 最新売上日の長さ
...
// 	↑↑仕様変更（2005.08.01）↑↑
//      ↓↓2006.06.21 fanglh カスタマイズ修正↓↓
	private String syori_flg = null;// 処理区分
	private String saihin_hacyu_dt = null;// 最新発注日
	private String saihin_shiire_dt = null;// 最新仕入日
	private String saihin_eriage_dt = null;// 最新売上日
	private String non_act_soshin_dt = null;// NON_ACT送信日
//      ↑↑2006.06.21 fanglh カスタマイズ修正↑↑
	...
	/**
	 * mst300201_TanpinTenToriatukaiBeanを１件のみ抽出したい時に使用する
	 */
	public static mst300201_TanpinTenToriatukaiBean getmst300201_TanpinTenToriatukaiBean(DataHolder dataHolder)
	{
		mst300201_TanpinTenToriatukaiBean bean = null;
		BeanHolder beanHolder = new BeanHolder( new mst300201_TanpinTenToriatukaiDM() );
		try
		{
			Iterator ite = beanHolder.getPageBeanListFromDataHolder( dataHolder ).iterator();
			// １件以上存在する時はまず保存する
			if( ite.hasNext() )
				bean = (mst300201_TanpinTenToriatukaiBean )ite.next();
			// ２件以上存在する時はＮＵＬＬにする
			if( ite.hasNext() )
				bean = null;
		}
...
//      ↓↓2006.06.21 fanglh カスタマイズ修正↓↓
	// 	処理区分に対するセッターとゲッターの集合
	public boolean setSyoriFlg(String syori_flg)
	{
	...
	{
		return cutString(this.syori_flg,SYORI_FLG_MAX_LENGTH);
	}
	public String getSyoriFlgString()
	{
		return "'" + cutString(this.syori_flg,SYORI_FLG_MAX_LENGTH) + "'";
	}
	public String getSyoriFlgHTMLString()
	{
		return HTMLStringUtil.convert(cutString(this.syori_flg,SYORI_FLG_MAX_LENGTH));
	}

	// 	最新発注日に対するセッターとゲッターの集合
	public boolean setSaihinHacyuDt(String saihin_hacyu_dt)
	{
		this.saihin_hacyu_dt = saihin_hacyu_dt;
		return true;
	}
	public String getSaihinHacyuDt()
	{
		return cutString(this.saihin_hacyu_dt,SAISHIN_HACYU_DT_MAX_LENGTH);
	}
	public String getSaihinHacyuDtString()
	{
		return "'" + cutString(this.saihin_hacyu_dt,SAISHIN_HACYU_DT_MAX_LENGTH) + "'";
	}
	public String getSaihinHacyuDtHTMLString()
	{
		return HTMLStringUtil.convert(cutString(this.saihin_hacyu_dt,SAISHIN_HACYU_DT_MAX_LENGTH));
	}

	// 	最新仕入日に対するセッターとゲッターの集合 
	public boolean setSaihinShiireDt(String saihin_shiire_dt)
	{
		this.saihin_shiire_dt = saihin_shiire_dt;
		return true;
	}
	public String getSaihinShiireDt()
	{
		return cutString(this.saihin_shiire_dt,SAISHIN_SHIIRE_DT_MAX_LENGTH);
	}
	public String getSaihinShiireDtString()
	{
		return "'" + cutString(this.saihin_shiire_dt,SAISHIN_SHIIRE_DT_MAX_LENGTH) + "'";
	}
	public String getSaihinShiireDtHTMLString()
	{
		return HTMLStringUtil.convert(cutString(this.saihin_shiire_dt,SAISHIN_SHIIRE_DT_MAX_LENGTH));
	}

	// 	最新売上日に対するセッターとゲッターの集合
	public boolean setSaihinEriageDt(String saihin_eriage_dt)
	{
		this.saihin_eriage_dt = saihin_eriage_dt;
		return true;
	}
	public String getSaihinEriageDt()
	{
		return cutString(this.saihin_eriage_dt,SAISHIN_ERIAGE_DT_MAX_LENGTH);
	}
	public String getSaihinEriageDtString()
	{
		return "'" + cutString(this.saihin_eriage_dt,SAISHIN_ERIAGE_DT_MAX_LENGTH) + "'";
	}
	public String getSaihinEriageDtHTMLString()
	{
		return HTMLStringUtil.convert(cutString(this.saihin_eriage_dt,SAISHIN_ERIAGE_DT_MAX_LENGTH));
	}

	// 	NON_ACT送信日に対するセッターとゲッターの集合
...
//      ↓↓2006.06.21 fanglh カスタマイズ修正↓↓
			+ "  saihin_hacyu_dt = " + getSaihinHacyuDtString()
			+ "  saihin_shiire_dt = " + getSaihinShiireDtString()
			+ "  saihin_eriage_dt = " + getSaihinEriageDtString()
			+ "  non_act_soshin_dt = " + getNonActSoshinDtString()
//      ↑↑2006.06.21 fanglh カスタマイズ修正↑↑
...
// 			↑↑仕様追加による変更（2005.06.29）↑↑
// 			↓↓仕様変更（2005.08.01）↓↓
			+ "  donyu_st_dt = " + getDonyuStDt()
			+ "  donyu_ed_dt = " + getDonyuEdDt()
// 			↑↑仕様変更（2005.08.01）↑↑
	 ...
	 * @return mst300201_TanpinTenToriatukaiBean コピー後のクラス
	 */
	public mst300201_TanpinTenToriatukaiBean createClone()
	{
...
//      ↓↓2006.06.21 fanglh カスタマイズ修正↓↓
		bean.setSaihinHacyuDt(this.saihin_hacyu_dt);
		bean.setSaihinShiireDt(this.saihin_shiire_dt);
		bean.setSaihinEriageDt(this.saihin_eriage_dt);
		bean.setNonActSoshinDt(this.non_act_soshin_dt);
//      ↑↑2006.06.21 fanglh カスタマイズ修正↑↑
...
// 		↑↑仕様変更（2005.08.01）↑↑
	...
	public boolean equals( Object o )
	{
		...
		if( !( o instanceof mst300201_TanpinTenToriatukaiBean ) ) return false;

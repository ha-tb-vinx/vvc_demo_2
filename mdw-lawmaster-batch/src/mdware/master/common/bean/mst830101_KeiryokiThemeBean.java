/**
 * <p>タイトル: 新ＭＤシステム（マスター管理）計量器テーママスタのレコード格納用クラス</p>
 * <p>説明: 新ＭＤシステムで使用する計量器テーママスタのレコード格納用クラス(DmCreatorにより自動生成)</p>
 ...
 * @version 1.0(2005/03/03)初版作成
 */
public class mst830101_KeiryokiThemeBean
{
	...
	/**
	 * RKeiryokiThemeBeanを１件のみ抽出したい時に使用する
	 */
	public static mst830101_KeiryokiThemeBean getRKeiryokiThemeBean(DataHolder dataHolder)
	{
		mst830101_KeiryokiThemeBean bean = null;
		BeanHolder beanHolder = new BeanHolder( new mst830101_KeiryokiThemeDM() );
		try
		{
			Iterator ite = beanHolder.getPageBeanListFromDataHolder( dataHolder ).iterator();
			//１件以上存在する時はまず保存する
			if( ite.hasNext() )
				bean = (mst830101_KeiryokiThemeBean )ite.next();
			//２件以上存在する時はＮＵＬＬにする
			if( ite.hasNext() )
				bean = null;
		}
	 ...
	 * @return RKeiryokiThemeBean コピー後のクラス
	 */
	public mst830101_KeiryokiThemeBean createClone()
	{
	...
	public boolean equals( Object o )
	{
		...
		if( !( o instanceof mst830101_KeiryokiThemeBean ) ) return false;

/**
 * <p>タイトル: 新ＭＤシステム（マスター管理）キーPLUマスタ画面の検索結果レコード格納用クラス</p>
 * <p>説明: 新ＭＤシステムで使用するキーPLUマスタ画面の検索結果レコード格納用クラス(DmCreatorにより自動生成)</p>
 ...
 * @version 1.0(2005/04/13)初版作成
 */
public class mst710301_KeyPluListBean
{
	private static final StcLog stcLog = StcLog.getInstance();

	public static final int KANJI_NA_MAX_LENGTH = 20;				//キーPLUの正式名称（漢字）の長さ
	...
	private String kanji_na = null;				//キーPLUの正式名称（漢字）
	...
	/**
	 * mst710301_KeyPluListBeanを１件のみ抽出したい時に使用する
	 */
	public static mst710301_KeyPluListBean getmst710301_KeyPluListBean(DataHolder dataHolder)
	{
		mst710301_KeyPluListBean bean = null;
		BeanHolder beanHolder = new BeanHolder( new mst710301_KeyPluListDM() );
		try
		{
			Iterator ite = beanHolder.getPageBeanListFromDataHolder( dataHolder ).iterator();
			//１件以上存在する時はまず保存する
			if( ite.hasNext() )
				bean = (mst710301_KeyPluListBean )ite.next();
			//２件以上存在する時はＮＵＬＬにする
			if( ite.hasNext() )
				bean = null;
		}
	...
	{
		return
			  "  keyplu_no_nb = " + getKeypluNoNbString()
	 ...
	 * @return mst710301_KeyPluListBean コピー後のクラス
	 */
	public mst710301_KeyPluListBean createClone()
	{
		mst710301_KeyPluListBean bean = new mst710301_KeyPluListBean();
		bean.setKeypluNoNb(this.getKeypluNoNb());
		bean.setKanjiNa(this.getKanjiNa());
	...
	public boolean equals( Object o )
	{
		...
		if( !( o instanceof mst710301_KeyPluListBean ) ) return false;

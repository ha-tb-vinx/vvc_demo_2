/**
 * <p>タイトル: 新ＭＤシステム（マスタ管理）店別品種マスタのDMクラス</p>
 * <p>説明: 	新ＭＤシステムで使用する店別品種マスタのDMクラス(DmCreatorにより自動生成)</p>
...
public class mst790101_TenbetsuHinsyuHinsyuDM extends DataModule
{
	 ...
	 * コンストラクタ
	 */
	public mst790101_TenbetsuHinsyuHinsyuDM()
	{
		...
		throws SQLException
	{
		...
		mst790101_TenbetsuHinsyuHinsyuBean bean = new mst790101_TenbetsuHinsyuHinsyuBean();

		bean.setSentaku( "" );
		bean.setTenpoCd( rest.getString("TENPO_CD") );
		bean.setTenpoNa( rest.getString("KANJI_NA") );
	...
	public String getSelectSql( Map map )
	{
		...
		sb.append("SELECT ");
		sb.append("		TENPO.TENPO_CD AS TENPO_CD, ");
		sb.append("		TENPO.KANJI_NA AS KANJI_NA, ");
		//ＰＣ区分　デフォルト：選択状態
		...
		sb.append("	   ON TENPO.TENPO_CD = TEN.TENPO_CD ");
		sb.append("	  AND TEN.HINSYU_CD = '" + conv.convertWhereString( (String)map.get("hinsyu_cd") ) + "'");
		...
		sb.append("	 	  		 WHERE TAIKEI.HINSYU_CD = '" + conv.convertWhereString( (String)map.get("hinsyu_cd") ) + "'");
		...
		sb.append("	   ON C.HINSYU_CD = '" + conv.convertWhereString( (String)map.get("hinsyu_cd") ) + "'");
		sb.append("	  AND TENPO.TENPO_CD = C.TENPO_CD ");

		sb.append("	WHERE 1 = 1 ");
		sb.append("	  AND TENPO.TENPO_KB = '1' ");
		sb.append("	  AND TENPO.DELETE_FG = '0' ");
		sb.append("	  AND TENPO.SNDST_PLU_DT < '" + conv.convertWhereString( (String)map.get("mst_date") ) + "'");
		...
		if( map.get("syori_kb").equals("0"))
		{
			sb.append("AND TEN.HINSYU_CD IS NULL ");
			whereStr = andStr;
		}
	...
	public String getInsertSql( Object beanMst )
	{
		...
		mst790101_TenbetsuHinsyuHinsyuBean bean = (mst790101_TenbetsuHinsyuHinsyuBean)beanMst;
		StringBuffer sb = new StringBuffer();

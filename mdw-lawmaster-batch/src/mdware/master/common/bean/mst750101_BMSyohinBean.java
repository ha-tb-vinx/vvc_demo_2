/**
 * <p>タイトル: 新ＭＤシステム（マスター管理）BM商品マスタのレコード格納用クラス</p>
 * <p>説明: 新ＭＤシステムで使用するBM商品マスタのレコード格納用クラス(DmCreatorにより自動生成)</p>
 ...
 * @version 1.0(2005/04/19)初版作成
 */
public class mst750101_BMSyohinBean
{
	...
	/**
	 * mst750101_BMSyohinBeanを１件のみ抽出したい時に使用する
	 */
	public static mst750101_BMSyohinBean getmst750101_BMSyohinBean(DataHolder dataHolder)
	{
		mst750101_BMSyohinBean bean = null;
		BeanHolder beanHolder = new BeanHolder( new mst750101_BMSyohinDM() );
		try
		{
			Iterator ite = beanHolder.getPageBeanListFromDataHolder( dataHolder ).iterator();
			//１件以上存在する時はまず保存する
			if( ite.hasNext() )
				bean = (mst750101_BMSyohinBean )ite.next();
			//２件以上存在する時はＮＵＬＬにする
			if( ite.hasNext() )
				bean = null;
		}
	 ...
	 * @return RBANDLEMIXSYOHINBean コピー後のクラス
	 */
	public mst750101_BMSyohinBean createClone()
	{
	...
	public boolean equals( Object o )
	{
		...
		if( !( o instanceof mst750101_BMSyohinBean ) ) return false;

...
REM ----------------------------------------------------------------------------
REM  JOB-ID      : MSTB992
REM  BATCH-ID    : MSTB992-408
REM  IF          : MDware => POS
REM  IF-NAME     : VAT NUMBER
REM  FUNCTION    : RENAME
REM  CRATOR      : G.SASAKI
REM  CREATE DATE : 2017/01/14
REM ----------------------------------------------------------------------------
REM  1-1. COMMON-SET
REM ----------------------------------------------------------------------------
REM (1)BATCH-ID 
SET BAT_ID=MSTB992-408
...
SET TARGET_DIR=H:\POS-HongKong\q5\mdware\download\Q\

REM (2)RENAME-TARGET-FILE-NAME
SET TARGETFILENAME=Q*_noncompleted

REM (3)COPY-FROM-DIR 
SET SND_FROM_DIR=E:\mdware_mkh\datas\MKH\POS\q5\mdware\download\Q\
...
echo [RENAME VAT-NUMBER FILE] >>%LOG_DIR%%LOG_FILE%
...
for /F %%A in ('dir /b %SND_FROM_DIR%%TARGETFILENAME%') do (
    ...
    IF !RENAME_CHECK_STATUS! == OK (
        ...
        IF !ERRORLEVEL! NEQ 0 (
            ...
            dir /a-d %TARGET_DIR%!RENAMEFIL! >>%LOG_DIR%%LOG_FILE%
            echo. >>%LOG_DIR%%LOG_FILE%
        )
    ) ELSE (
        echo. >>%LOG_DIR%%LOG_FILE%

...
REM ----------------------------------------------------------------------------
REM  JOB-ID      : MSTB991
REM  BATCH-ID    : MSTB991-020
REM  IF          : MDware => FAST
REM  IF-NAME     :【FAST向けI/F】アイテム一覧ファイルの送信処理
...
SET INFILENAME=fast_item_list_????????.csv

REM (3)COPY-TO-DIR 
SET SND_TO_DIR=Z:\

REM (4)COPY-TO-FILE-NAME
SET OUTFILENAME=fast_item_list_????????.csv

...
REM ----------------------------------------------------------------------------
REM  JOB-ID      : MSTB992
REM  BATCH-ID    : MSTB992-302
REM  IF          : MDware => POS
REM  IF-NAME     : Department
REM  FUNCTION    : COPY
...
REM   送信ファイル   例: T0706010.001_noncompleted
REM ----------------------------------------------------------------------------
REM (1)COPY-FROM-DIR 
SET SND_FROM_DIR=E:\mdware_mkh\datas\MKH\POS\q5\mdware\download\T\

REM (2)COPY-FROM-FILE-NAME
REM SET INFILENAME=T*_noncompleted

REM (3)COPY-TO-DIR 
REM SET SND_TO_DIR=H:\POS-HongKong\q5\mdware\download\T\

SET SND_TO_DIR=%1
SET INFILENAME=T*%SND_TO_DIR:~-5,1%.%SND_TO_DIR:~-4,3%_noncompleted
...
echo [COPY DEPT FILE] >>%LOG_DIR%%LOG_FILE%
...
CD %BATCHF_HOME%
rem exit 1
type nul > %LOG_DIR%POS-IF-NG\T-File-NG.Store-CD-%SND_TO_DIR:~-7,6%
exit /b

...
/**
 *
 * <p>タイトル: TorikomiMaeHDao.java クラス</p>
 * <p>説明　: Hレコード取込前処理</p>
 * <p>著作権: Copyright (c) 2016</p>
 * <p>会社名: VINX</p>
 * @Version 1.00  (2016.11.23) k.Hyo FIVI対応(#2726)
 * @Version 1.01  (2017.01.16) T.Kamei FIVI対応(#3609)
 *
 */
public class TorikomiMaeHDao implements DaoIf {

    /** バッチ処理名 */
    private static final String DAO_NAME = "Hレコード取込前処理";

    /** 削除SQL */
    private static final String DEL_SQL = "TRUNCATE TABLE TMP_POS_H_HAMPER_WK";

    /** 登録先テーブル名称(POSジャーナル一時データ) */
    private static final String TMP_POS_H_HAMPER_WK = "POSジャーナル（H_HAMPER）一時データ_WK";
    ...
    public void executeDataAccess(DaoInvokerIf invoker) throws Exception {
        ...
        try {
            ...
            // 開始
            invoker.infoLog(strUserId + "　：　" + TMP_POS_H_HAMPER_WK + "の出力を開始します。");
            preparedStatementExIns = invoker.getDataBase().prepareStatement(getEigyoDtInsertSql());
            insertCount = preparedStatementExIns.executeUpdate();

            // ログ出力
            invoker.infoLog(strUserId + "　：　" + insertCount + "件の" + TMP_POS_H_HAMPER_WK + "を出力しました。");
            invoker.infoLog(strUserId + "　：　" + TMP_POS_H_HAMPER_WK + "の出力を終了します。");
    ...
    private String getEigyoDtInsertSql() {
        ...
        sql.append("  TMP_POS_H_HAMPER_WK ");
        sql.append("( ");
        sql.append("  COMMAND ");
        sql.append("  ,STORE ");
        sql.append("  ,POS ");
        ...
        sql.append("  ,GST_TAX ");
        sql.append("  ,DISC4_AMT ");
        // 2017.01.16 T.Kamei FIVI対応 #3609 Mod(S)
        //sql.append("  ,SUBSTR(MATOME_COLUMN,   1, 120) AS ITEM_NAME_RECEIPT ");
        sql.append("  ,SUBSTR(MATOME_COLUMN,   1,  40) AS ITEM_NAME_RECEIPT ");
        //sql.append("  ,SUBSTR(MATOME_COLUMN, 121,  24) AS ITEM_UOM_RECEIPT ");
        sql.append("  ,SUBSTR(MATOME_COLUMN,  41,   8) AS ITEM_UOM_RECEIPT ");
        //sql.append("  ,SUBSTR(MATOME_COLUMN, 145,  10) AS PRCCHG_NO ");
        sql.append("  ,SUBSTR(MATOME_COLUMN,  49,  10) AS PRCCHG_NO ");
        //sql.append("  ,SUBSTR(MATOME_COLUMN, 155,  10) AS PRCCHG1_NO ");
        sql.append("  ,SUBSTR(MATOME_COLUMN,  59,  10) AS PRCCHG1_NO ");
        //sql.append("  ,SUBSTR(MATOME_COLUMN, 165,  11) AS PRCCHG2_NO ");
        sql.append("  ,SUBSTR(MATOME_COLUMN,  69,  11) AS PRCCHG2_NO ");
        //sql.append("  ,SUBSTR(MATOME_COLUMN, 176,  10) AS PRCCHG3_NO ");
        sql.append("  ,SUBSTR(MATOME_COLUMN,  80,  10) AS PRCCHG3_NO ");
        //sql.append("  ,SUBSTR(MATOME_COLUMN, 186,   1) AS PRIVILEGE_MEM ");
        sql.append("  ,SUBSTR(MATOME_COLUMN,  90,   1) AS PRIVILEGE_MEM ");
        //sql.append("  ,SUBSTR(MATOME_COLUMN, 187,   1) AS OVER_WRITE_FLAG ");
        sql.append("  ,SUBSTR(MATOME_COLUMN,  91,   1) AS OVER_WRITE_FLAG ");
        //sql.append("  ,SUBSTR(MATOME_COLUMN, 188,  13) AS HAMPER_ITEM_CD ");
        sql.append("  ,SUBSTR(MATOME_COLUMN,  92,  13) AS HAMPER_ITEM_CD ");
        //sql.append("  ,SUBSTR(MATOME_COLUMN, 201,   9) AS SUPERVISOR_ID ");
        sql.append("  ,SUBSTR(MATOME_COLUMN, 108,   6) AS SUPERVISOR_ID ");
        // 2017.01.16 T.Kamei FIVI対応 #3609 Mod(E)
        sql.append("FROM ");
        sql.append("  TMP_POS_H_HAMPER_WK_BEFORE ");

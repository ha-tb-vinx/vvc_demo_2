...
/**
 * <p>タイトル: MaseterCheckTmpPosETenderDao クラス</p>
 * <p>説明:マスタチェック（Eレコード）</p>
 ...
 * @see なし
 */
public class MaseterCheckTmpPosETenderDao implements DaoIf {

    // バッチ処理名
    private static final String BATCH_NAME = "マスタチェック（Eレコード）";
    String userLocal = ResorceUtil.getInstance().getPropertie("USER_LOCAL");

    // バッチID
    private static final String BATCH_ID = "URIB012920";
    ...
    /** 削除SQL */
    private static final String DEL_SQL = "TRUNCATE TABLE WK_POS_E_TENDER";
    ...
    public void executeDataAccess(DaoInvokerIf invoker) throws Exception {
        ...
        try {
            ...
            // ログ出力
            invoker.infoLog(strUserID + "　：　" + insertCount + "件のEレコードチェックデータを追加しました。");
    ...
    /**
     * マスタチェック（Eレコード）SQLを取得する
     *
     * @return マスタチェック（Eレコード）
     */
    private String getMaseterCheckTmpPosETenderDaoSql(String dbServerTime) {
        StringBuilder sql = new StringBuilder();

        sql.append("INSERT /*+ APPEND */ INTO WK_POS_E_TENDER ( ");
        sql.append("     COMMAND ");
        sql.append("    ,STORE ");
        sql.append("    ,POS ");
        sql.append("    ,TRANS_NO ");
        ...
        sql.append("SELECT ");
        sql.append("     TPET.COMMAND ");
        sql.append("    ,TPET.STORE ");
        sql.append("    ,TPET.POS ");
        sql.append("    ,TPET.TRANS_NO ");
        sql.append("    ,TPET.CASHIER_ID ");
        sql.append("    ,TPET.FORMAT ");
        sql.append("    ,TPET.ETYPE ");
        sql.append("    ,TPET.PYMT_TYPE ");
        sql.append("    ,TPET.PYMT_TYPE2 ");
        sql.append("    ,TPET.PAYMENT_SALES ");
        sql.append("    ,TPET.PAYMENT_COUNT ");
        sql.append("    ,TPET.EIGYO_DT ");
        sql.append("    ,? ");
        sql.append("    ,CASE ");
        sql.append("         WHEN DPET.STORE IS NOT NULL THEN '1' ");
        sql.append("         WHEN (SEISAN_ST_DT IS NULL OR SEISAN_ED_DT IS NULL) OR (SUBSTR(TPET.EIGYO_DT, 1, 8) < SEISAN_ST_DT OR SEISAN_ED_DT < SUBSTR(TPET.EIGYO_DT, 1, 8)) THEN '4' ");
...
//        sql.append("         WHEN ISAN_SEISAN_STATE_FG <> '0' THEN '9'  ");
        ...
        sql.append("    ,? ");
        sql.append("FROM ");
        sql.append("    TMP_POS_E_TENDER TPET ");
        sql.append("LEFT OUTER JOIN ");
        sql.append("    DT_POS_E_TENDER DPET ");
        sql.append("ON ");
        sql.append("    TPET.STORE = DPET.STORE AND ");
        sql.append("    TPET.POS = DPET.POS AND ");
        //#4875 Mod J.Endo 2017.05.01 (S)
//        sql.append("    TPET.TRANS_NO = DPET.TRANS_NO AND ");
        sql.append("    NVL(TPET.TRANS_NO,' ') = NVL(DPET.TRANS_NO,' ') AND ");
        //#4875 Mod J.Endo 2017.05.01 (E)
        sql.append("    TPET.EIGYO_DT = DPET.EIGYO_DT ");
        //#5399 Add X.Liu 2017.06.14 (S)
        sql.append("    AND TPET.CASHIER_ID = DPET.CASHIER_ID ");//チェッカーコード
        sql.append("    AND TPET.PYMT_TYPE = DPET.PYMT_TYPE ");//支払い種別コード
        sql.append("    AND TPET.PYMT_TYPE2 = DPET.PYMT_TYPE2 ");//サブ支払い種別コード
        //#5399 Add X.Liu 2017.06.14 (E)
        // ＜SUB店舗マスタ＞
        ...
        sql.append("    ) RT ");
        sql.append("ON ");
        sql.append("    RT.TENPO_CD = '00' || TPET.STORE ");
        // ＜SUB店別精算状況データ＞
        //#5399 Mod X.Liu 2017.06.14 (S)
//        sql.append("LEFT JOIN ");
//        sql.append("    ( ");
...
//        sql.append("    ) DTSS ");
//        sql.append("ON ");
//        sql.append("    DTSS.KEIJO_DT = SUBSTR(TPET.EIGYO_DT, 1, 8) AND ");
//        sql.append("    DTSS.TENPO_CD = '00' || TPET.STORE ");
        sql.append("  LEFT JOIN ( ");
        sql.append("    SELECT");
        sql.append("      KEIJO_DT");
        sql.append("      , TENPO_CD");
        ...
        sql.append("      COMP_CD = ?");
        sql.append("  ) DTCSS ");
        sql.append("    ON DTCSS.KEIJO_DT = SUBSTR(TPET.EIGYO_DT, 1, 8) ");
        sql.append("    AND DTCSS.TENPO_CD = '00' || TPET.STORE ");
        sql.append("    AND DTCSS.CHECKER_CD = TPET.CASHIER_ID ");
        sql.append("  LEFT JOIN ( ");
        sql.append("    SELECT");
        ...
        sql.append("      COMP_CD = ?");
        sql.append("  ) RC ");
        sql.append("    ON RC.START_DT <= SUBSTR(TPET.EIGYO_DT, 1, 8) ");
        sql.append("    AND RC.END_DT >= SUBSTR(TPET.EIGYO_DT, 1, 8)");
        //#5399 Mod X.Liu 2017.06.14 (E)

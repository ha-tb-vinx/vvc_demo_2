...
/**
 * <p>タイトル: MaseterCheckTmpPosBTotalDao クラス</p>
 * <p>説明:マスタチェック（Bレコード）</p>
 ...
 * @Version 1.03 (2017.04.10) X.Liu   FIVI対応(#4553)
 * @Version 1.04 (2017.06.14) X.Liu   FIVI対応(#5399)
 *
 * @see なし
 */
public class MaseterCheckTmpPosBTotalDao implements DaoIf {

    // バッチ処理名
    private static final String BATCH_NAME = "マスタチェック（Bレコード）";
    String userLocal = ResorceUtil.getInstance().getPropertie("USER_LOCAL");

    // バッチID
    private static final String BATCH_ID = "URIB012680";
    ...
    /** 削除SQL */
    private static final String DEL_SQL = "TRUNCATE TABLE WK_POS_B_TOTAL";
    //private static final String DEL_SQL = "TRUNCATE TABLE TMP_POS_B_TOTAL_CHECK_DATA";
    ...
    public void executeDataAccess(DaoInvokerIf invoker) throws Exception {
        ...
        try {
            ...
            preparedStatementExIns = invoker.getDataBase().prepareStatement(getMaseterCheckTmpPosBTotalDaoSql(dbServerTime));
            ...
            // ログ出力
            invoker.infoLog(strUserID + "　：　" + insertCount + "件のBレコードチェックデータを追加しました。");
    ...
    /**
     * マスタチェック（Bレコード）SQLを取得する
     *
     * @return マスタチェック（Bレコード）
     */
    private String getMaseterCheckTmpPosBTotalDaoSql(String dbServerTime) {
        StringBuilder sql = new StringBuilder();

        // 2016/11/10 VINX Y.Itaki FIVI(#2725) MOD(S)

        sql.append("INSERT /*+ APPEND */ INTO WK_POS_B_TOTAL ( ");
        sql.append("     COMMAND ");
        sql.append("    ,STORE ");
        sql.append("    ,POS ");
        sql.append("    ,TRANS_NO ");
        ...
        sql.append("SELECT ");
        sql.append("     TPBT.COMMAND ");
        sql.append("    ,TPBT.STORE ");
        sql.append("    ,TPBT.POS ");
        sql.append("    ,TPBT.TRANS_NO ");
        sql.append("    ,TPBT.CASHIER_ID ");
        sql.append("    ,TPBT.FORMAT ");
        sql.append("    ,TPBT.BTYPE ");
        sql.append("    ,TPBT.REC_TOT ");
        sql.append("    ,TPBT.NTAXABLE ");
        sql.append("    ,TPBT.TAX_AMT ");
        sql.append("    ,TPBT.TOT_AMT ");
        sql.append("    ,TPBT.EIGYO_DT ");
        sql.append("    ,TPBT.TORI_TIME ");
        sql.append("    ,TPBT.CHANGE ");
        sql.append("    ,TPBT.MEM_CARD ");
        sql.append("    ,TPBT.REFUND_ORI_TRANS# ");
        sql.append("    ,TPBT.REFUND_ORI_STORE ");
        sql.append("    ,TPBT.REFUND_ORI_SALES_DATE ");
        sql.append("    ,TPBT.REFUND_REASON ");
        sql.append("    ,TPBT.REFUND_DESCRIPTION ");
        sql.append("    ,? ");
        sql.append("    ,CASE ");
        // #3344 2016.12.16 T.Kamei Mod (S)
        //sql.append("         WHEN DPBT.STORE IS NULL THEN '0' ");
        //sql.append("         ELSE '1' ");
        sql.append("         WHEN DPBT.STORE IS NOT NULL THEN '1' ");
        sql.append("         WHEN (SEISAN_ST_DT IS NULL OR SEISAN_ED_DT IS NULL) OR (SUBSTR(TPBT.EIGYO_DT, 1, 8) < SEISAN_ST_DT OR SEISAN_ED_DT < SUBSTR(TPBT.EIGYO_DT, 1, 8)) THEN '4' ");
...
//        sql.append("         WHEN ISAN_SEISAN_STATE_FG <> '0' THEN '9' ");
        ...
        sql.append("    ,? ");
        sql.append("FROM ");
        sql.append("    TMP_POS_B_TOTAL TPBT ");
        sql.append("LEFT OUTER JOIN ");
        sql.append("    DT_POS_B_TOTAL DPBT ");
        sql.append("ON ");
        sql.append("    TPBT.STORE = DPBT.STORE AND ");
        sql.append("    TPBT.POS = DPBT.POS AND ");
        sql.append("    TPBT.TRANS_NO = DPBT.TRANS_NO AND ");
        sql.append("    TPBT.EIGYO_DT = DPBT.EIGYO_DT ");
        ...
        sql.append("    ) RT ");
        sql.append("ON ");
        sql.append("    RT.TENPO_CD = '00' || TPBT.STORE ");
        // ＜SUB店別精算状況データ＞
        //#5399 Mod X.Liu 2017.06.14 (S)
//        sql.append("LEFT JOIN ");
//        sql.append("    ( ");
...
//        sql.append("    ) DTSS ");
//        sql.append("ON ");
//        sql.append("    DTSS.KEIJO_DT = SUBSTR(TPBT.EIGYO_DT, 1, 8) AND ");
//        sql.append("    DTSS.TENPO_CD = '00' || TPBT.STORE ");
        // #3344 2016.12.16 T.Kamei Add (E)
        ...
        sql.append("      COMP_CD = ?");
        sql.append("  ) DTCSS ");
        sql.append("    ON DTCSS.KEIJO_DT = SUBSTR(TPBT.EIGYO_DT, 1, 8) ");
        sql.append("    AND DTCSS.TENPO_CD = '00' || TPBT.STORE ");
        sql.append("    AND DTCSS.CHECKER_CD = TPBT.CASHIER_ID ");
        sql.append("  LEFT JOIN ( ");
        sql.append("    SELECT");
        ...
        sql.append("      COMP_CD = ?");
        sql.append("  ) RC ");
        sql.append("    ON RC.START_DT <= SUBSTR(TPBT.EIGYO_DT, 1, 8) ");
        sql.append("    AND RC.END_DT >= SUBSTR(TPBT.EIGYO_DT, 1, 8)");
        //#5399 Mod X.Liu 2017.06.14 (E)
        return sql.toString();
    }

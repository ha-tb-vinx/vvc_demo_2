...
/**
 *  <P>タイトル: ShinTanpinSummarySabunFTPSoshinDao クラス</p>
 *  <P>説明: 新単品サマリ(差分）FTP送信処理です。</p>
 *  <P>著作権: Copyright (c) 2009</p>
 *  <P>会社名: Vinculum Japan Corporation</P>
 *  @author HX.Su
 *  @version 1.0 
 */
public class ShinTanpinSummarySabunFTPSoshinDao implements DaoIf {

    // バッチ処理名
    private static final String BATCH_NAME = "新単品サマリ(差分）FTP送信処理";
    // サーバアドレス
    ...
    // 送信先ディレクトリ
    private static final String PATH_SEND_TANPINSAM_SABUN_SYOHIN = FiResorceUtility.getPropertie(UriResorceKeyConstant.PATH_SEND_TANPINSAM_SABUN_SYOHIN);
    // 送信元ディレクトリ
    ...
    public void executeDataAccess(DaoInvokerIf invoker) throws Exception {
        ...
        // ログ出力
        invoker.infoLog(strUserID + "　：　新単品サマリ(差分）FTP送信処理を開始します。");

        // FTPファイルの存在をチェックする
        if (!FiFileUtility.pathFileExists(PATH_SEND, FILE_TANPINSAM_SABUN)) {
            // ファイルが無い場合、ログ出力
            invoker.warnLog(strUserID + "　：　新単品サマリファイル(" + FILE_TANPINSAM_SABUN + ")が存在しません。新単品サマリ(差分）FTP送信処理は行われませんでした。");
        ...
        // 送信先ディレクトリ
        inputBean.setDestDirName(PATH_SEND_TANPINSAM_SABUN_SYOHIN);
        // 送信先ファイル名
        inputBean.setDestFileName(FILE_TANPINSAM_SABUN);
        // 送信元ディレクトリ
        inputBean.setSourceDirName(PATH_SEND);
        // 送信元ファイル名
        inputBean.setSourceFileName(FILE_TANPINSAM_SABUN);
        ...
        // ログ出力
        invoker.infoLog(strUserID + "　：　新単品サマリ(差分）FTP送信処理を終了します。");
    ...
    public static void main(String[] arg) {
        try {
            DaoIf dao = new ShinTanpinSummarySabunFTPSoshinDao();

...
import jp.co.vinculumjapan.mdware.uriage.dao.batch.input.InshizeiKensuDataTorikomiDaoInputBean;
...
/**
 *  <P>タイトル: InshizeiKensuDataTorikomiDao クラス</p>
 *  <P>説明: 印紙税件数データ取込処理</p>
 ...
 *  @version 1.0 
 */
public class InshizeiKensuDataTorikomiDao implements DaoIf {

    // バッチ処理名
    private static final String BATCH_NAME = "印紙税件数データ取込処理";
    // 更新対象テーブル名 
    private static final String TABLE_NAME = "WK_TEN_INSHIZEI";

    // 入力ビーン
    private InshizeiKensuDataTorikomiDaoInputBean inputBean = null;
    ...
    public void executeDataAccess(DaoInvokerIf invoker) throws Exception {
        ...
        // ログ出力
        invoker.infoLog(strUserID + "　：　印紙税件数データ取込処理を開始します。");
        ...
        if (!FiFileUtility.pathFileExists(FiResorceUtility.getPropertie(UriResorceKeyConstant.PATH_KIKAN_RECV_ONMEMO_LOADER), inputBean.getDataFileName())) {
            // CSVファイルが無い場合、ログ出力
            invoker.warnLog(strUserID + "　：　印紙税件数データ(" + inputBean.getDataFileName() + ")が存在しません。印紙税件数データ取込処理は行われませんでした。");
            invoker.infoLog(strUserID + "　：　印紙税件数データ取込処理を終了します。");
            throw new DaoException("データ取込処理にて例外エラー発生。");
        }

        // ログ出力
        invoker.infoLog(strUserID + "　：　SQL*Loaderで、店別印紙税対象ワークへのデータ取込処理を開始します。");
        ...
        if (outBean.getResultCd() != 0) {
            // 正常終了以外は異常終了
            invoker.warnLog(strUserID + "　：　SQL*Loaderで、店別印紙税対象ワークへのデータ取込処理にて例外エラーが発生しました。");
            throw new DaoException("データ取込処理にて例外エラー発生。");
        }

        // ログ出力
        invoker.infoLog(strUserID + "　：　SQL*Loaderで、店別印紙税対象ワークへのデータ取込処理を終了します。取込件数は" + outBean.getResultCount() + "件です。");
        invoker.infoLog(strUserID + "　：　印紙税件数データ取込処理を終了します。");
    }
    ...
    public void setInputObject(Object input) throws Exception {
        inputBean = (InshizeiKensuDataTorikomiDaoInputBean) input;
    }

    public static void main(String[] arg) {
        try {
            DaoIf dao = new InshizeiKensuDataTorikomiDao();

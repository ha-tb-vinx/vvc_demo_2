...
/**
 *
 * <p>タイトル: DtHamperUriageJisekiCreateDao.java クラス</p>
 * <p>説明　: ハンパー売上実績データ作成（卸）</p>
 ...
 *
 */
public class DtHamperUriageJisekiCreateDao implements DaoIf {

    // プロパティファイルより法人コード取得
    private static final String DAO_NAME = "ハンパー売上実績データ作成（卸）";
    /** 登録先テーブル名称(ハンパー商品売上実績データ作成) */
    private static final String DT_HAMPER_URIAGE_JISEKI_NAME = "ハンパー売上実績データ";
    
    private static final String BATCH_ID = "URIB301020";
    ...
    /**
     * ハンパー商品売上実績データ作成用SQLを取得する
     *
     * @return ハンパー商品売上実績データ作成用SQL
     */
    private String getHamperKouseiSyohinUriageJisekiInsertSql() {
        StringBuilder sql = new StringBuilder();
        sql.append("MERGE ");
        sql.append("INTO DT_HAMPER_URIAGE_JISEKI T1 ");
        sql.append("  USING ( ");
        sql.append("SELECT");
        sql.append("  SH.KEIJO_DT AS URIAGE_DT");
        sql.append("  , SH.TENPO_CD AS TENPO_CD");
        sql.append("  , SH.HAMPER_SYOHIN_CD AS HAMPER_SYOHIN_CD");
        sql.append("  , MIN(RT.KANJI_NA) AS TENPO_NA");
        sql.append("  , MIN(SH.HAMPER_SYOHIN_NA) AS SYOHIN_NA");
        sql.append("  , CASE ");
        sql.append("    WHEN MIN(SH.TEIKAN_KB) = '1' ");
        sql.append("    THEN SUM(SH.NOUHIN_SURYO_QT) ");
        sql.append("    ELSE SUM(SH.NOUHIN_JYURYO_QT * 1000) ");
        sql.append("    END AS URIAGE_QT");
        //#5943 Mod X.Liu 2017.09.19 (S)
//        sql.append("  , SUM(SH.BAIKA_ZEIKOMI_VL) AS URIAGE_VL");
        sql.append("  , SUM(SH.BAIKA_ZEINUKI_VL) AS URIAGE_VL");
        ...
        sql.append("  , MIN(SH.OROSHI_BAITANKA_ZEINUKI_VL) AS HAMPER_BAITANKA_VL");
        sql.append("  , MIN(SH.TEIKAN_KB) AS TEIKAN_KB");
        //#5943 Mod X.Liu 2017.09.19 (S)
//        sql.append("  , ( ");
//        sql.append("    SUM(SH.BAIKA_ZEIKOMI_VL) / ( ");
//        sql.append("      CASE ");
//        sql.append("        WHEN MIN(SH.TEIKAN_KB) = '1' ");
//        sql.append("        THEN SUM(SH.NOUHIN_SURYO_QT) ");
//        sql.append("        ELSE SUM(SH.NOUHIN_JYURYO_QT) * 1000 ");
...
//        sql.append("  ) AS HAMPER_NEBIKIGO_BAITANKA ");
        sql.append("      , CASE ");
        sql.append("        WHEN MIN(SH.TEIKAN_KB) = '1' ");
        sql.append("        AND SUM(SH.NOUHIN_SURYO_QT) = 0 ");
        sql.append("        THEN 0 ");
        sql.append("        WHEN (MIN(SH.TEIKAN_KB) != '1' OR MIN(SH.TEIKAN_KB) IS NULL)");
        sql.append("        AND SUM(SH.NOUHIN_JYURYO_QT) * 1000 = 0 ");
        sql.append("        THEN 0 ");
        sql.append("        ELSE ( ");
        sql.append("          SUM(SH.BAIKA_ZEINUKI_VL) / ( ");
        sql.append("            CASE ");
        sql.append("              WHEN MIN(SH.TEIKAN_KB) = '1' ");
        sql.append("              THEN SUM(SH.NOUHIN_SURYO_QT) ");
        sql.append("              ELSE SUM(SH.NOUHIN_JYURYO_QT) * 1000 ");
        ...
        //#5943 Mod X.Liu 2017.09.19 (E)
        sql.append("FROM");
        sql.append("  WK_OROSHI_SYUKA_HAM_SYO SH ");
        sql.append("  LEFT JOIN R_TENPO RT ");
        sql.append("    ON SH.TENPO_CD = RT.TENPO_CD ");
        sql.append("GROUP BY");
        sql.append("  SH.HAMPER_SYOHIN_CD");
        sql.append("  , SH.KEIJO_DT");
        sql.append("  , SH.TENPO_CD");
        sql.append("  , SH.OROSHI_BAITANKA_ZEINUKI_VL");
        sql.append("  ) WPHH ");
        sql.append("    ON ( ");
        sql.append("      T1.HAMPER_SYOHIN_CD = WPHH.HAMPER_SYOHIN_CD ");
        sql.append("      AND T1.URIAGE_DT = WPHH.URIAGE_DT ");
        sql.append("      AND T1.TENPO_CD = WPHH.TENPO_CD ");
        sql.append("      AND T1.HAMPER_BAITANKA_VL = WPHH.HAMPER_BAITANKA_VL");
        sql.append("    ) WHEN MATCHED THEN UPDATE ");
        sql.append("SET");
        sql.append("  URIAGE_QT = NVL(T1.URIAGE_QT, 0) + NVL(WPHH.URIAGE_QT, 0)");
        sql.append("  , URIAGE_VL = NVL(T1.URIAGE_VL, 0) + NVL(WPHH.URIAGE_VL, 0)");
        //#5943 Mod X.Liu 2017.09.19 (S)
//        sql.append("  , HAMPER_NEBIKIGO_BAITANKA = (NVL(T1.URIAGE_VL, 0) + NVL(WPHH.URIAGE_VL, 0)) / (NVL(T1.URIAGE_QT, 0) + NVL(WPHH.URIAGE_QT, 0))");
        sql.append("  , HAMPER_NEBIKIGO_BAITANKA = CASE ");
        sql.append("    WHEN NVL(T1.URIAGE_QT, 0) + NVL(WPHH.URIAGE_QT, 0) = 0 ");
        sql.append("    THEN 0 ");
        sql.append("    ELSE (NVL(T1.URIAGE_VL, 0) + NVL(WPHH.URIAGE_VL, 0)) / (NVL(T1.URIAGE_QT, 0) + NVL(WPHH.URIAGE_QT, 0))");
        sql.append("    END");
        //#5943 Mod X.Liu 2017.09.19 (E)

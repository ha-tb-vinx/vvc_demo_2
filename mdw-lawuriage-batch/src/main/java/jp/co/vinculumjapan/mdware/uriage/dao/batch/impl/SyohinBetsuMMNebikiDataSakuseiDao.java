...
/**
 *
 * <p>タイトル: SyohinBetsuMMNebikiDataSakuseiDao.java クラス</p>
 * <p>説明　: 商品別MM値引データ作成処理</p>
 * <p>著作権: Copyright (c) 2013</p>
 * <p>会社名: VINX</p>
 * @Version 1.00 (2014.12.04) chou グローバル化対応 通貨対応
 *
 */
public class SyohinBetsuMMNebikiDataSakuseiDao implements DaoIf {
    ...
    /** バッチ処理名 */
    private static final String DAO_NAME = "商品別MM値引データ作成処理";
    /** 登録先テーブル名称 */
    ...
    public void executeDataAccess(DaoInvokerIf invoker) throws Exception {
        ...
        try {
            ...
            // ログ出力
            invoker.infoLog(strUserId + "　：　" + INS_TABLE_NAME + "(MM値引)の追加を開始します。");
            ...
            preparedStatementExIns.setString(5, UriageCommonConstants.RIYU_SYOSAI_KB_MIX_MATCH_BAIHEN);
            ...
            // ログ出力
            invoker.infoLog(strUserId + "　：　" + insertCount + "件の" + INS_TABLE_NAME + "(MM値引)を追加しました。");

            // ログ出力
            invoker.infoLog(strUserId + "　：　" + INS_TABLE_NAME + "(MM値引)の追加を終了します。");
    ...
    private String getWkNejogePosInsertSql() {
        ...
        sql.append("        ,DTS.TAX_RT ");
        sql.append("        ,DTS.MST_BAITANKA_VL ");
        sql.append("        ,DTS.NEBIKI_MM_VL AS BAIHEN_VL ");
        sql.append("        ,ABS(DTS.NEBIKI_MM_QT) AS SURYO_QT ");
        sql.append("    FROM ");
        sql.append("        DT_TANPIN_SEISAN DTS ");
        sql.append("    WHERE ");
        sql.append("        DTS.COMP_CD              = ?    AND ");
        sql.append("        DTS.KEIJO_DT             = ?    AND ");
        sql.append("        DTS.NEBIKI_MM_VL        <> 0    AND ");
        sql.append("        DTS.BUNRUI1_CD          <> ? ");
        sql.append("    ) DTS ");
        sql.append("WHERE ");
    ...
    public static void main(String[] args) {
        try {
            DaoIf dao = new SyohinBetsuMMNebikiDataSakuseiDao();

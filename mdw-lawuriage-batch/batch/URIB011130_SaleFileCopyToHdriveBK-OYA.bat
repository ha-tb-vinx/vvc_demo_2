...
REM  BATCH-ID    : URIB011130_SaleFileCopyToHdriveBK-OYA
REM  IF          : POS => MDware
...
REM (1)BATCH-ID 
SET BAT_ID=URIB011130_SaleFileCopyToHdriveBK-OYA
...
echo [1][START][COPY SALE-FILE]                         >>%LOG_DIR%%LOG_FILE%
...
SET /a num=0
   for /D %%F in (%DirPath%*) do (
      ...
      call @filter.shellscript.baseDirectory.value@\batch\URIB011130_SaleFileCopyToHdriveBK.bat !senddir!\ !num!
      IF !ERRORLEVEL! NEQ 0 goto ABEND
     )

echo [1][E N D][COPY SALE-FILE]                         >>%LOG_DIR%%LOG_FILE%

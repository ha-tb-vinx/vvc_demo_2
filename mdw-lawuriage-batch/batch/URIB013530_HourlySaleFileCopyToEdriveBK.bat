@echo off
REM ----------------------------------------------------------------------------
REM  JOB-ID      : URIB013530
REM  BATCH-ID    : URIB013530
REM  IF          : POS => MDware
REM  IF-NAME     : HourlySaleFile
REM  FUNCTION    : COPY to Edrive BackUp
REM  CRATOR      : G.SASAKI
...
REM ----------------------------------------------------------------------------
REM (1)BATCH-ID 
SET BAT_ID=URIB013530_HourlySaleFile_CopyToEdriveBkup
...
REM ----------------------------------------------------------------------------
REM (1)COPY-FROM-DIR 
SET SND_FROM_DIR=@filter.env.baseDirectory.value@\datas@filter.env.value@\POS\q5\mdware\upload\R\

REM (2)COPY-FROM-FILE-NAME
SET INFILENAME=R???????.???

REM (3)COPY-TO-DIR 
mkdir %SND_FROM_DIR%backup\%BKUPDIR%
SET SND_TO_DIR=%SND_FROM_DIR%backup\%BKUPDIR%\

@echo off
REM ----------------------------------------------------------------------------
REM  JOB-ID      : URIB013540
REM  BATCH-ID    : URIB013540
REM  IF          : POS => MDware
...
REM ----------------------------------------------------------------------------
REM (1)BATCH-ID 
SET BAT_ID=URIB013540_HourlySaleFile_Merge
...
REM ----------------------------------------------------------------------------
REM (1)FROM-DIR 
SET FROM_DIR=@filter.env.baseDirectory.value@\datas@filter.env.value@\POS\q5\mdware\upload\R\wk\

REM (2)COPY-FROM-FILE-NAME
SET INFILENAME=R???????.???

REM (3)TO-DIR 
SET TO_DIR=@filter.env.baseDirectory.value@\datas@filter.env.value@\POS\q5\mdware\upload\R\

REM (4)COPY-TO-FILE-NAME
SET OUTFILENAME=ALL_HourlySALE_FILE.txt
...
echo [1-2][NG][NO-FROM-FIL] >>%LOG_DIR%%LOG_FILE%
